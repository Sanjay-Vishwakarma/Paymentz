...
//            preparedStatement.setString(2, accountId);
    ...
    public List<TransactionVO> getRollingReserveTransaction(String memberId,String accountId,String terminalIds,String rollingReserveStartDate, String rollingReserveEndDate)
    {
        ...
        try
        {
            String query = null;
            con = Database.getRDBConnection();
            query = "SELECT t.terminalid,t.trackingid,t.description,t.paymodeid,t.cardtypeid,FROM_UNIXTIME(t.dtstamp) as transactiondate,t.amount,t.captureamount,t.currency,t.STATUS,t.firstname,t.lastname,bd.isRollingReserveReleased FROM transaction_common as t JOIN bin_details as bd where t.toid=? and t.accountid IN ("+accountId+")and FROM_UNIXTIME(t.dtstamp) >=? and FROM_UNIXTIME(t.dtstamp) <=? and t.terminalid IN("+terminalIds+") and bd.isRollingReserveReleased=? and bd.icicitransid=t.trackingid and t.STATUS IN ('chargebackreversed','settled', 'reversed', 'chargeback') order by t.terminalid";
