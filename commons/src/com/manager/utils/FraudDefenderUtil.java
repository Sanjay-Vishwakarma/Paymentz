...
public class FraudDefenderUtil
{
    ...
    public void blackListTransactionOfFraudDefenderRefund(Hashtable transHash,CommonValidatorVO commonValidatorVO) throws PZDBViolationException
    {
        ...
        String reason=(String) transHash.get("reason");
        String remark=(String) transHash.get("remark");
        if(functions.isValueNull(ccnum))
        {
            firstSix=functions.getFirstSix(ccnum);
            lastFour=functions.getLastFour(ccnum);
            boolean isCardNotBlocked=transactionHelper.isCardBlocked(firstSix, lastFour,commonValidatorVO);
            if(isCardNotBlocked)
                blacklistManager.insertBlacklistedCards(firstSix,lastFour,actionExecutorId,actionExecutorName,"", BlacklistEnum.Fraud_Received.toString());
        }
