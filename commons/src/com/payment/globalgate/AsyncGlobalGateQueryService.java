...
import com.payment.PZTransactionStatus; import com.payment.apco.core.ApcoPayPaymentProcess;
 ...
 */
public class AsyncGlobalGateQueryService
{
    //private Session session;
    private static TransactionLogger transactionLogger = new TransactionLogger(AsyncGlobalGateQueryService.class.getName());
    ...
    public static  AsyncGlobalGateQueryService asyncGlobalGateQueryService = null;
    public   AsyncGlobalGateQueryService()
    {
    ...
    public AsyncGlobalGateQueryService(String trackingid,String accountId,String paymentId,String isService,String afterExecuteTime,String delayTime,String maxExecutionTime)
    {
        //timer = new Timer();
        //timer.schedule(new RemindTask(), seconds*1000);

        callAsyncGlobalGateQuery(trackingid, accountId, paymentId, isService, afterExecuteTime, delayTime, maxExecutionTime);
    ...
    public static AsyncGlobalGateQueryService getInstance()
    {
        if(asyncGlobalGateQueryService !=null)
        {
            return asyncGlobalGateQueryService;
        }
        else
        {
            return new AsyncGlobalGateQueryService();
        }
    }



    public Future<String> callAsyncGlobalGateQuery(String trackingid,String accountId,String paymentId,String isService,String afterExecuteTime,String delayTime,String maxExecutionTime)
    {
        return executorService.submit(new GlobalGateQueryCallable(trackingid,accountId,paymentId,isService, afterExecuteTime, delayTime, maxExecutionTime));
    }
    public class GlobalGateQueryCallable implements Callable<String>
    {
        ...
        private GlobalGateQueryCallable(String trackingid,String accountId,String paymentId,String isService,String afterExecuteTime,String delayTime,String maxExecutionTime)
        {
            this.paymentId = paymentId;
            this.trackingID = trackingid;
            this.isService = isService;
        ...
        public String call() throws Exception
        {
            try
            {
                ...
                exec.scheduleAtFixedRate(new Runnable()
                {
                    ...
                    {
                        // do stuff
                        transactionLogger.error("------Inside GlobalGatePayQueryService run() method for trackingID--------" + trackingid);
                ...
                {
                    transactionLogger.error("Async GlobalGateQuery awaitTermination---"+trackingid);
                    exec.shutdownNow();
                }
            }
            catch (InterruptedException ie)
            {
                transactionLogger.error("InterruptedException in GlobalGateQuery---",ie);
            }
    ...
    {
        transactionLogger.error("------Inside GlobalgatePayQueryService callQuery method--------");
        ...
        transactionLogger.error("Inside call for GlobalGatePayQueryService---"+trackingid+"--");
        ...
        GlobalGatePaymentProcess paymentProcess = new GlobalGatePaymentProcess();
            ...
            if (transactionDetailsVO != null)
            {
                ...
                if (dbStatus.equalsIgnoreCase(PZTransactionStatus.AUTH_STARTED.toString()))
                {
                    if ("N".equals(isService))
                    {
                        ...
                        genericResponseVO = (CommResponseVO) globalGatePaymentGateway.processInquiry(commRequestVO);
                        transType = "Auth";
                    }
                    else
                    {
                    ...
                    if (genericResponseVO != null && dbStatus.equalsIgnoreCase(PZTransactionStatus.AUTH_STARTED.toString()))
                    {
                        ...
                        if ("success".equals(transactionStatus))
                        {
                            ...
                            else
                            {
                                ...
                                dbBuffer.append("update transaction_common set paymentid='" + transactionId + "' ,status='authsuccessful',remark='" + ESAPI.encoder().encodeForSQL(me, message) + "' where trackingid = " + trackingid);
                                con = Database.getConnection();
                                Database.executeUpdate(dbBuffer.toString(), con);
                                paymentProcess.actionEntry(trackingid, amount, ActionEntry.ACTION_AUTHORISTION_SUCCESSFUL, ActionEntry.STATUS_AUTHORISTION_SUCCESSFUL, genericResponseVO, null, auditTrailVO);
                        ...
                        if (!"pending".equalsIgnoreCase(transactionStatus) && !"PSP - Ticket Unavailable!".equalsIgnoreCase(message) && !"ENROLLED".equalsIgnoreCase(message))
                        {
                            ...
                            {
                                transactionLogger.error("inside sending notification from AsyncGlobalgateQuery---" + notificationUrl);
