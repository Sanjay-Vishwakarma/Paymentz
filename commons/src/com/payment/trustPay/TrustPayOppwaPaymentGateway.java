...
public class TrustPayOppwaPaymentGateway extends AbstractPaymentGateway
{
    public static final String GATEWAY_TYPE = "trustpay";
    final static ResourceBundle RB = LoadProperties.getProperty("com.directi.pg.trustpay");
    private static TransactionLogger transactionLogger = new TransactionLogger(TrustPayOppwaPaymentGateway.class.getName());

    public TrustPayOppwaPaymentGateway(String accountId)
    {
    ...
    {
        transactionLogger.error("TrustPayOppwaPaymentGateway:: inside processSale()");
        String reqType = "DB";
     ...
     /*   if (functions.isValueNull(commMerchantVO.getHostUrl()))
       ...
       // }
        ...
        transactionLogger.error("TrustPayOppwaPaymentGateway:: merchantId(entity id) ---" + merchantId);
        String authorizationToken = gatewayAccount.getFRAUD_FTP_PASSWORD();
        transactionLogger.debug("TrustPayOppwaPaymentGateway:: authorizationToken ---" + authorizationToken);
        ...
        try
        {
            if (is3dSupported.equalsIgnoreCase("Y") || is3dSupported.equalsIgnoreCase("O"))
            {
                ...
                transactionLogger.error("TrustPayOppwaPaymentGateway:: SaleRequest --- for " + trackingID + "----" + sale3DRequestlog);

                transactionLogger.error("recurringType----->" + recurringType);

                if (isTest)
                {
                    //  sale3DRequest.append("&testMode=" + testMode);
                ...
                transactionLogger.error("TrustPayOppwaPaymentGateway:: sale response ---for " + trackingID + "----" + response);
            ...
            else
            {
                ...
                transactionLogger.error("recurringType----->" + recurringType);
                transactionLogger.error("TrustPayOppwaPaymentGateway:: SaleRequest --- for " + trackingID + "----" + saleRequestLog);
    ...
    {
        CommRequestVO commRequestVO = (CommRequestVO) requestVO;
        transactionLogger.error("TrustPayOppwaPaymentGateway:: inside processAuthentication()");
        String reqType = "PA";
        Comm3DResponseVO commResponseVO = new Comm3DResponseVO();
        PayonOppwaUtils payonOppwaUtils = new PayonOppwaUtils();
        Functions functions = new Functions();
        ...
        try
        {
            if (is3dSupported.equalsIgnoreCase("Y") || is3dSupported.equalsIgnoreCase("O"))
            {
                ...
                transactionLogger.error("TrustPayOppwaPaymentGateway:: SaleRequest--- processAuthentication --- for " + trackingID + "----" + auth3DRequestlog);
                transactionLogger.error("recurringType----->" + recurringType);
                ...
                transactionLogger.error("TrustPayOppwaPaymentGateway:: sale response--- processAuthentication---for " + trackingID + "----" + response);
            ...
            {
                transactionLogger.error("Inside non-3D condition");
                StringBuffer PreAuthRequest = new StringBuffer();
                PreAuthRequest.append("authentication.entityId=" + merchantId
                        + "&amount=" + transactionAmount
                        + "&currency=" + transactionCurrency
                        + "&paymentBrand=" + cardType
                        + "&paymentType=" + reqType
                ...
                StringBuffer PreAuthRequestLog = new StringBuffer();
                PreAuthRequestLog.append("authentication.entityId=" + merchantId
                        + "&amount=" + transactionAmount
                        + "&currency=" + transactionCurrency
                        + "&paymentBrand=" + cardType
                        + "&paymentType=" + reqType
                ...
                if ("INITIAL".equals(recurringType) || "Manual".equalsIgnoreCase(recurringType))
                {
                    PreAuthRequest.append("&recurringType=INITIAL&createRegistration=true");
                }


                transactionLogger.error("recurringType----->" + recurringType);
                transactionLogger.error("TrustPayOppwaPaymentGateway:: PreAuthRequest --- for " + trackingID + "----" + PreAuthRequestLog);

                if (isTest)
                {
                    transactionLogger.error("inside isTest-----" + RB.getString("TEST_URL"));
                    response = payonOppwaUtils.doPostHTTPSURLConnectionClient(RB.getString("TEST_URL"), authorizationToken, PreAuthRequest.toString());
                }
                else
                {
                    transactionLogger.error("inside isLive-----" + RB.getString("LIVE_URL"));
                    response = payonOppwaUtils.doPostHTTPSURLConnectionClient(RB.getString("LIVE_URL"), authorizationToken, PreAuthRequest.toString());
                }

                transactionLogger.error("TrustPayOppwaPaymentGateway:: PreAuth response ---for " + trackingID + "----" + response);
    ...
    {
        transactionLogger.error("TrustPayOppwaPaymentGateway:: inside processCapture()");
        String reqType = "CP";
        String time="";
        NPayOnResponseVO responseVO = null;
        PayonOppwaUtils payonOppwaUtils = new PayonOppwaUtils();
        ...
        try
        {
            ...
            transactionLogger.error("TrustPayOppwaPaymentGateway :: Capture request-----for "+trackingID+ "----" + request);
            if (isTest)
            {
                //    request.append("&testMode="+testMode);
                transactionLogger.error("TrustPayOppwaPaymentGateway :: inside isTest -----for "+trackingID+ "----" + RB.getString("TEST_URL")+"/"+  previousTransactionId);
            ...
            {
                transactionLogger.error("TrustPayOppwaPaymentGateway :: inside isTest -----for "+trackingID+ "----" + RB.getString("LIVE_URL")+"/"+  previousTransactionId);
            ...
            transactionLogger.error("TrustPayOppwaPaymentGateway :: Capture response-----for "+trackingID+ "----" + response);
        ...
        catch (JsonMappingException e)
        {
            ...
            PZExceptionHandler.raiseTechnicalViolationException(TrustPayOppwaPaymentGateway.class.getName(), "processCapture()", null, "common", "Technical Exception while capture transaction", PZTechnicalExceptionEnum.JSON_MAPPING_EXCEPTION, null, e.getMessage(), e.getCause());
        }
        catch (JsonParseException e)
        {
            transactionLogger.error("JsonParseException---" +trackingID + "----" ,  e);
            PZExceptionHandler.raiseTechnicalViolationException(TrustPayOppwaPaymentGateway.class.getName(), "processCapture()", null, "common", "Technical Exception while capture transaction", PZTechnicalExceptionEnum.JSON_PARSE_EXCEPTION, null, e.getMessage(), e.getCause());
        }
        catch (IOException e)
        {
            transactionLogger.error("IOException---" +trackingID + "----" ,  e);
            PZExceptionHandler.raiseTechnicalViolationException(TrustPayOppwaPaymentGateway.class.getName(), "processCapture()", null, "common", "Technical Exception while capture transaction", PZTechnicalExceptionEnum.IOEXCEPTION, null, e.getMessage(), e.getCause());
        }
        return commResponseVO;
    }
    ...
    {
        transactionLogger.error("TrustPayOppwaPaymentGateway:: inside processVoid()");
        String reqType = "RV";
        String time="";
        NPayOnResponseVO responseVO = null;
        PayonOppwaUtils payonOppwaUtils = new PayonOppwaUtils();
        ...
        try
        {
            ...
            transactionLogger.error("TrustPayOppwaPaymentGateway :: cancel request-----for "+trackingID+ "----" + request);
            if (isTest)
            {
                // request.append("&testMode="+testMode);
                transactionLogger.error("TrustPayOppwaPaymentGateway :: inside isTest -----for "+trackingID+ "----" + RB.getString("TEST_URL")+"/"+  previousTransactionId);
            ...
            {
                transactionLogger.error("TrustPayOppwaPaymentGateway :: inside isLive -----for "+trackingID+ "----" + RB.getString("LIVE_URL")+"/"+  previousTransactionId);
                response = payonOppwaUtils.doPostHTTPSURLConnectionClient(RB.getString("LIVE_URL")+"/"+ previousTransactionId, authorizationToken, request.toString());
            }

            transactionLogger.error("TrustPayOppwaPaymentGateway :: cancel response-----for "+trackingID+ "----" + response);
        ...
        {
            transactionLogger.error("JsonMappingException---" +trackingID + "----" ,  e);
            PZExceptionHandler.raiseTechnicalViolationException(TrustPayOppwaPaymentGateway.class.getName(), "processVoid()", null, "common", "Technical Exception while void transaction", PZTechnicalExceptionEnum.JSON_MAPPING_EXCEPTION, null, e.getMessage(), e.getCause());
        }
        catch (JsonParseException e)
        {
            transactionLogger.error("JsonParseException---" +trackingID + "----" ,  e);
            PZExceptionHandler.raiseTechnicalViolationException(TrustPayOppwaPaymentGateway.class.getName(), "processVoid()", null, "common", "Technical Exception while void transaction", PZTechnicalExceptionEnum.JSON_PARSE_EXCEPTION, null, e.getMessage(), e.getCause());
        }
        catch (IOException e)
        {
            transactionLogger.error("IOException---" +trackingID + "----" ,  e);
            PZExceptionHandler.raiseTechnicalViolationException(TrustPayOppwaPaymentGateway.class.getName(), "processVoid()", null, "common", "Technical Exception while void transaction", PZTechnicalExceptionEnum.IOEXCEPTION, null, e.getMessage(), e.getCause());
        }
        return commResponseVO;
    }
    ...
    {
        transactionLogger.error("TrustPayOppwaPaymentGateway :: inside processRefund()");
        String reqType = "RF";
        String time="";
        ...
        try
        {
            StringBuffer request =new StringBuffer(
                    "entityId=" + merchantId +
                            ...
                            "&currency=" + commTransactionDetailsVO.getCurrency() +
                            "&paymentType=" + reqType);
            transactionLogger.error("TrustPayOppwaPaymentGateway :: refund request-----for "+trackingID+ "----" + request);
            ...
            transactionLogger.error("TrustPayOppwaPaymentGateway :: refund response-----for "+trackingID+ "----" + response);
        ...
        {
            transactionLogger.error("JsonMappingException---" +trackingID + "----" ,  e);
            PZExceptionHandler.raiseTechnicalViolationException(TrustPayOppwaPaymentGateway.class.getName(), "processRefund()", null, "common", "Technical Exception while refunding transaction", PZTechnicalExceptionEnum.JSON_MAPPING_EXCEPTION, null, e.getMessage(), e.getCause());
        }
        catch (JsonParseException e)
        {
            transactionLogger.error("JsonParseException---" +trackingID + "----" ,  e);
            PZExceptionHandler.raiseTechnicalViolationException(TrustPayOppwaPaymentGateway.class.getName(), "processRefund()", null, "common", "Technical Exception while refunding transaction", PZTechnicalExceptionEnum.JSON_PARSE_EXCEPTION, null, e.getMessage(), e.getCause());
        }
        catch (IOException e)
        {
            transactionLogger.error("IOException---" +trackingID + "----" ,  e);
            PZExceptionHandler.raiseTechnicalViolationException(TrustPayOppwaPaymentGateway.class.getName(), "processRefund()", null, "common", "Technical Exception while refunding transaction", PZTechnicalExceptionEnum.IOEXCEPTION, null, e.getMessage(), e.getCause());
        }
        return commResponseVO;
    }
    ...
    public GenericResponseVO processInquiry(GenericRequestVO requestVO) throws PZTechnicalViolationException, PZDBViolationException, PZConstraintViolationException
    {
        ...
        transactionLogger.error("TrustPayOppwaPaymentGateway :: inside processInquiry()");
        Functions functions=new Functions();
        PayonOppwaUtils payonOppwaUtils=new PayonOppwaUtils();
        Comm3DResponseVO commResponseVO = new Comm3DResponseVO();
        CommRequestVO commRequestVO = (CommRequestVO) requestVO;
        ...
        try
        {
            ...
            transactionLogger.error("TrustPayOppwaPaymentGateway :: inquiry response-----for "+previousTransactionId+ "----" + response);
        ...
        catch (JsonMappingException e)
        {
            ...
            PZExceptionHandler.raiseTechnicalViolationException(TrustPayOppwaPaymentGateway.class.getName(), "processInquiry()", null, "common", "Technical Exception while inquiry transaction", PZTechnicalExceptionEnum.JSON_MAPPING_EXCEPTION, null, e.getMessage(), e.getCause());
        }
        catch (JsonParseException e)
        {
            transactionLogger.error("JsonParseException---"  + "----" ,  e);
            PZExceptionHandler.raiseTechnicalViolationException(TrustPayOppwaPaymentGateway.class.getName(), "processInquiry()", null, "common", "Technical Exception while inquiry transaction", PZTechnicalExceptionEnum.JSON_PARSE_EXCEPTION, null, e.getMessage(), e.getCause());
        }
        catch (IOException e)
        {
            transactionLogger.error("IOException---"  + "----" ,  e);
            PZExceptionHandler.raiseTechnicalViolationException(TrustPayOppwaPaymentGateway.class.getName(), "processInquiry()", null, "common", "Technical Exception while inquiry transaction", PZTechnicalExceptionEnum.IOEXCEPTION, null, e.getMessage(), e.getCause());
        }
        return commResponseVO;
    }
    public GenericResponseVO processRebilling(String trackingID, GenericRequestVO requestVO) throws PZGenericConstraintViolationException
    {
        transactionLogger.error("TrustPayOppwaPaymentGateway:: inside processRecurring()");
        String reqType = "DB";
        CommRequestVO commRequestVO = (CommRequestVO) requestVO;
        CommTransactionDetailsVO commTransactionDetailsVO = commRequestVO.getTransDetailsVO();
        Comm3DResponseVO commResponseVO = new Comm3DResponseVO();
        ...
        try
        {
            ...
            transactionLogger.error("recurringType----->" + recurringType);
            transactionLogger.error("TrustPayOppwaPaymentGateway:: RecurringRequest --- for "+trackingID+ "----"+RecurringRequest);
            ...
            transactionLogger.error("TrustPayOppwaPaymentGateway:: Recurring response ---"+response);
            NPayOnResponseVO responseVO = null;
            ObjectMapper objectMapper = new ObjectMapper();
            objectMapper.configure(DeserializationConfig.Feature.FAIL_ON_UNKNOWN_PROPERTIES, false);
            responseVO = objectMapper.readValue(response, NPayOnResponseVO.class);
...
/*
        ...
        {
            transactionLogger.error("JsonMappingException-----"+ trackingID + "----" , e);
            PZExceptionHandler.raiseTechnicalViolationException(TrustPayOppwaPaymentGateway.class.getName(), "processRecurring()", null, "common", "Technical Exception while recurring transaction", PZTechnicalExceptionEnum.JSON_MAPPING_EXCEPTION, null, e.getMessage(), e.getCause());
        }
        catch (JsonParseException e)
        {
            transactionLogger.error("JsonParseException-----"+ trackingID + "----" , e);
            PZExceptionHandler.raiseTechnicalViolationException(TrustPayOppwaPaymentGateway.class.getName(), "processRecurring()", null, "common", "Technical Exception while recurring transaction", PZTechnicalExceptionEnum.JSON_PARSE_EXCEPTION, null, e.getMessage(), e.getCause());
        }
        catch (IOException e)
        {
            transactionLogger.error("IOException---"+ trackingID + "----" , e);
            PZExceptionHandler.raiseTechnicalViolationException(TrustPayOppwaPaymentGateway.class.getName(), "processRecurring()", null, "common", "Technical Exception while recurring transaction", PZTechnicalExceptionEnum.IOEXCEPTION, null, e.getMessage(), e.getCause());
        }
