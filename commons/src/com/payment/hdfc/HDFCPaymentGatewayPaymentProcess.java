 ...
 */
public class HDFCPaymentGatewayPaymentProcess extends CommonPaymentProcess
{
    private static TransactionLogger transactionlogger = new TransactionLogger(HDFCPaymentGatewayPaymentProcess.class.getName());

    @Override
    public String get3DConfirmationForm(String trackingId, String ctoken, Comm3DResponseVO response3D)
    {
        transactionlogger.error("inside  HDFCPaymentGatewayPaymentProcess Form ---");

        String form = "<form name=\"creditcard_checkout\" method=\"GET\" action=\"" +response3D.getRedirectUrl()+ "\">" ;
        form  += "</form><script language=\"javascript\">window.location=\"" +response3D.getRedirectUrl()+ "\";</script>";

        transactionlogger.error("HDFCPaymentGatewayPaymentProcess Form---"+form.toString());
    ...
    {
        transactionlogger.error("inside HDFCPaymentGatewayPaymentProcess --->"+directKitResponseVO.getBankRedirectionUrl());
        directKitResponseVO.setBankRedirectionUrl(directKitResponseVO.getBankRedirectionUrl());
        transactionlogger.error("inside HDFCPaymentGatewayPaymentProcess getBankRedirectionUrl Process -->"+directKitResponseVO.getBankRedirectionUrl());
    ...
    {
        transactionlogger.debug("inside  HDFCPaymentGatewayPaymentProcess---> "+response3D.getRedirectUrl());
    ...
    {
        transactionlogger.debug("Entering ActionEntry for HDFCPaymentGatewayPaymentProcess");
        ...
        try
        {
            ...
            transactionlogger.error("SqlQuery HDFCPaymentGatewayPaymentProcess-----" + pstmt);

        }
        catch (SQLException se)
        {
            PZExceptionHandler.raiseDBViolationException(HDFCPaymentGatewayPaymentProcess.class.getName(), "actionEntryExtension()", null, "common", "Technical exception", PZDBExceptionEnum.SQL_EXCEPTION, null, se.getMessage(), se.getCause());
        }
        catch (SystemError systemError)
        {
            PZExceptionHandler.raiseDBViolationException(HDFCPaymentGatewayPaymentProcess.class.getName(), "actionEntryExtension()", null, "common", "Technical exception", PZDBExceptionEnum.DB_CONNECTION_ISSUE, null, systemError.getMessage(), systemError.getCause());
        }
        finally
        {
    ...
    private Hashtable getTransactionDetails(String trackingid, String status) throws PZDBViolationException
    {
        ...
        {
            transactionlogger.error("SystemError in HDFCPaymentGatewayPaymentProcess---",systemError);
            PZExceptionHandler.raiseDBViolationException(HDFCPaymentGatewayPaymentProcess.class.getName(), "getTransactionDetails()", null, "common", "DB Exception", PZDBExceptionEnum.DB_CONNECTION_ISSUE, null, systemError.getMessage(), systemError.getCause());
        }
        catch (SQLException e)
        {
            transactionlogger.error("SQLException in HDFCPaymentGatewayPaymentProcess---", e);
            PZExceptionHandler.raiseDBViolationException(HDFCPaymentGatewayPaymentProcess.class.getName(), "getTransactionDetails()", null, "common", "DB Exception", PZDBExceptionEnum.SQL_EXCEPTION, null, e.getMessage(), e.getCause());
        }
        finally
        {
