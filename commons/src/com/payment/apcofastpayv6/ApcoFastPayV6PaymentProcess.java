 ...
 */
public class ApcoFastPayV6PaymentProcess extends CommonPaymentProcess
{
    private static TransactionLogger transactionlogger = new TransactionLogger(ApcoFastPayV6PaymentProcess.class.getName());

    public String get3DConfirmationForm(CommonValidatorVO commonValidatorVO, Comm3DResponseVO response3D){
        transactionlogger.error("inside ApcoFastPayV6PaymentProcess  Form---");
        ...
        transactionlogger.error("ApcoFastPayV6PaymentProcess Form---"+form.toString());
    ...
    public void set3DResponseVO(DirectKitResponseVO directKitResponseVO, Comm3DResponseVO response3D, CommonValidatorVO commonValidatorVO)
    {
        ...
        transactionlogger.debug("inside  Fastpayv6 payment process---"+response3D.getUrlFor3DRedirect());
      ...
      *//*  while (keys.hasNext())
        ...
        while (keys.hasNext())
        {
            ...
            *//*asyncParameterVO    = new AsyncParameterVO();
            asyncParameterVO.setName(key);*//*
            transactionlogger.error("response3D.getRequestMap().get(key) "+response3D.getRequestMap().get(key));
            directKitResponseVO.setBankRedirectionUrl(response3D.getRequestMap().get(key));
