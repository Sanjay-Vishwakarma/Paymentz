...
import com.payment.Easebuzz.EasebuzzPaymentProcess;
...
import com.payment.Ecospend.EcospendPaymentProcess;
...
import com.payment.Gpay.GpayPaymentProcess;
...
import com.payment.aamarpay.AamarPayPaymentProcess;
import com.payment.airpay.AirpayPaymentGateway;
import com.payment.airpay.AirpayPaymentProcess;
import com.payment.airtel.AirtelUgandaPaymentProcess;
...
import com.payment.bnmquick.BnmQuickPaymentProcess;
import com.payment.continentPay.ContinentPayPaymentGateway;
import com.payment.continentPay.ContinentPayPaymentProcess;
import com.payment.flexepin.FlexepinPaymentGateway;
import com.payment.flexepin.FlexepinPaymentProcess;
import com.payment.mtn.MTNUgandaPaymentProcess;
...
import com.payment.apcofastpayv6.ApcoFastPayV6PaymentProcess;
import com.payment.apexpay.ApexPayPaymentGateway;
import com.payment.apexpay.ApexPayPaymentProcess;
import com.payment.appletree.AppleTreeCellulantPaymentGateway;
import com.payment.appletree.AppletreeCellulantPaymentProcess;
...
import com.payment.boltpay.BoltPayPaymentProcess;
...
import com.payment.cleveland.*;
import com.payment.cleveland.ClevelandPaymentProcess;
...
import com.payment.easypay.EasyPayPaymentProcess;
...
import com.payment.euroeximbank.EuroEximBankPaymentProcess;
...
import com.payment.globalgate.GlobalGatePaymentProcess;
...
import com.payment.hdfc.HDFCPaymentGatewayPaymentProcess;
...
import com.payment.lyra.LyraPaymentProcess;
...
import com.payment.boombill.BoomBillPaymentProcess;
import com.payment.infipay.InfiPayPaymentGateway;
import com.payment.infipay.InfiPayPaymentProcess;
import com.payment.onepay.OnePayPaymentGateway;
import com.payment.onepay.OnepayPaymentProcess;
...
import com.payment.payg.PayGPaymentProcess;
import com.payment.paygsmile.PayGSmilePaymentGateway;
import com.payment.paygsmile.PayGSmilePaymentProcess;
...
import com.payment.payu.PayUPaymentProcess;
...
import com.payment.powercash21.PowerCash21PaymentProcess;
...
import com.payment.qikpayv2.QikPayV2PaymentProcess;
...
import com.payment.smartfastpay.SmartFastPayPaymentProcess;
...
import com.payment.tigergateway.TigerGateWayPaymentProcess;
import com.payment.tigerpay.TigerPayPaymentGateway;
import com.payment.tigerpay.TigerPayPaymentProcess;
...
import com.payment.traxx.TRAXXPaymentProcess;
...
import com.payment.uba_mc.UBAMCPaymentProcess;
...
import com.payment.xcepts.XceptsPaymentProcess;
...
import com.payment.smartcode.SmartCodePayPaymentProcess;
...
public class PaymentProcessFactory
{
    ...
    private static AbstractPaymentProcess createPaymentProcess(String gateway)
    {
        ...
        else if(IMoneyPayPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new IMoneyPayPaymentProcess();

        }
        else if(WealthPayGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new WealthPayPaymentProcess();
        }
        else if(GlobalGatePaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new GlobalGatePaymentProcess();
        }
        else if(ApcoFastPayV6PaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new ApcoFastPayV6PaymentProcess();
        }
        else if(PayUPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new PayUPaymentProcess();

        }else if(PayGPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new PayGPaymentProcess();
        }else if(AirpayPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new AirpayPaymentProcess();
        }
       /* else if(ClevelandPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new ClevelandPaymentProcess();
        }*/
        else if(ApexPayPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new ApexPayPaymentProcess();
        }
        else if(ApexPayPaymentGateway.GATEWAY_TYPE.equals(gateway))
        ...
        else if(QikPayV2PaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new QikPayV2PaymentProcess();
        }
        else if(CashFlowsCaiboPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new CashFlowsCaiboPaymentProcess();
        }
        else if(EasyPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new EasyPayPaymentProcess();
        } else if(TigerPayPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new TigerPayPaymentProcess();
        }
        else if(BoltPayPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new BoltPayPaymentProcess();
        }

        else if(CajaRuralPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new CajaRuralPaymentProcess();
        }

        else if(TigerGateWayPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new TigerGateWayPaymentProcess();
        }
        else if(PayaidPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new PayaidPaymentProcess();
        }
        else if(OnePayPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new OnepayPaymentProcess();
        }
        else if(EuroEximBankPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new EuroEximBankPaymentProcess();
        }
        else if(AamarPayPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new AamarPayPaymentProcess();
        }
        else if(EcospendPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new EcospendPaymentProcess();
        }
        else if(OmnipayPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new OmnipayPaymentProcess();
        }
        else if(BoomBillPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new BoomBillPaymentProcess();
        }
        else if(PowerCash21PaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new PowerCash21PaymentProcess();
        }
        else if(PayTMPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new PayTMPaymentProcess();
        }else if(LyraPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new LyraPaymentProcess();
        }else if(UBAMCPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new UBAMCPaymentProcess();
        }
        else if(AirtelUgandaPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new AirtelUgandaPaymentProcess();
        }
        else if(HDFCPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new HDFCPaymentGatewayPaymentProcess();
        }
        else if(GpayPaymentzGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new GpayPaymentProcess();
        }
        else if(EaseBuzzPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new EasebuzzPaymentProcess();
        }
        else if(MTNUgandaPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new MTNUgandaPaymentProcess();
        }
        else if(ContinentPayPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new ContinentPayPaymentProcess();
        }
        else if(TransfrPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new TransactiumPaymentProcess();
        }
        else if(BitClearPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new BitClearPaymentProcess();
        }
        else if(XceptsPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new XceptsPaymentProcess();
        }
        else if(BnmQuickPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new BnmQuickPaymentProcess();
        }
        else if(FlexepinPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new FlexepinPaymentProcess();
        }
        else if(SmartCodePayPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new SmartCodePayPaymentProcess();
        }else if(SmartFastPayPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new SmartFastPayPaymentProcess();
        }
        else if(InfiPayPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new InfiPayPaymentProcess();
        }else if(TRAXXPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new TRAXXPaymentProcess();
        }
        else if(PayGSmilePaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new PayGSmilePaymentProcess();
        }
        else if(SwiffyPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
            return new SwiffyPaymentProcess();
        }
        else if(AlphapayPaymentGateway.GATEWAY_TYPE.equals(gateway))
        {
          return new AlphapayPaymentProcess();
        }
        else
