 ...
 */
public class AsyncEPaymentzPayoutQueryService
{

    private static TransactionLogger transactionLogger  = new TransactionLogger(AsyncEPaymentzPayoutQueryService.class.getName());
    ...
    public static AsyncEPaymentzPayoutQueryService asyncVervePayQueryService = null;
    public   AsyncEPaymentzPayoutQueryService()
    {
    ...
    public AsyncEPaymentzPayoutQueryService(String trackingid,String accountId,String paymentId,String afterExecuteTime,String delayTime,String maxExecutionTime)
    {
        //timer = new Timer();
        //timer.schedule(new RemindTask(), seconds*1000);
        transactionLogger.error("inside AsyncEPaymentzPayoutQueryService -------");
    ...
    public static AsyncEPaymentzPayoutQueryService getInstance()
    {
        ...
        {
            return new AsyncEPaymentzPayoutQueryService();
        }
    }
    ...
    {
        return executorService.submit(new EpaymentzQueryCallable(trackingid,accountId,paymentId, afterExecuteTime, delayTime, maxExecutionTime));
    }
    public class EpaymentzQueryCallable implements Callable<String>
    {
        ...
        private EpaymentzQueryCallable(String trackingid,String accountId,String paymentId,String afterExecuteTime,String delayTime,String maxExecutionTime)
        {
            this.paymentId = paymentId;
            this.trackingid = trackingid;
            this.accountId = accountId;
        ...
        public String call() throws Exception
        {
            try
            {
                ...
                exec.scheduleAtFixedRate(new Runnable()
                {
                    ...
                    {
                        // do stuff
                        transactionLogger.error("------Inside AsyncEPaymentzPayoutQueryService run() method for trackingid--------" + trackingid);
                ...
                {
                    transactionLogger.error("Async EPaymentz awaitTermination---"+trackingid);
                    exec.shutdownNow();
                }
            }
            catch (InterruptedException ie)
            {
                transactionLogger.error("InterruptedException in EPaymentzQuery---",ie);
            }
    ...
    {
        transactionLogger.error("------Inside AsyncEPaymentzPayoutQueryService callQuery method--------");
        Functions functions=new Functions();
        transactionLogger.error("paymentId--->"+paymentId);
        EasyPaymentzPaymentGateway easyPaymentzPaymentGateway=new EasyPaymentzPaymentGateway(accountId);
        transactionLogger.error("Inside call for AsyncEPaymentzPayoutQueryService---"+trackingid+"--");
        ...
        try
        {
            ...
            if (transactionDetailsVO != null)
            {
                ...
                if (dbStatus.equalsIgnoreCase(PZTransactionStatus.PAYOUT_STARTED.toString()))
                {
                    ...
                    genericResponseVO = (CommResponseVO) easyPaymentzPaymentGateway.processPayoutInquiry(trackingid, commRequestVO);
        ...
        {
            transactionLogger.error("systemError e-", systemError);
        }
        finally
        {
