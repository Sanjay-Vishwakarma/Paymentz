 ...
 */
public class AsyncMtNUgandaQueryService
{
    private static TransactionLogger transactionLogger = new TransactionLogger(AsyncMtNUgandaQueryService.class.getName());
    ...
    public static AsyncMtNUgandaQueryService AsyncMtNUgandaQueryService = null;
    public AsyncMtNUgandaQueryService()
    {
    ...
    public AsyncMtNUgandaQueryService(String trackingid, String accountId, String paymentId, String afterExecuteTime, String delayTime, String maxExecutionTime)
    {
        //timer = new Timer();
        //timer.schedule(new RemindTask(), seconds*1000);
        transactionLogger.error("inside AsyncMtNUgandaQueryService -------");
    ...
    public static AsyncMtNUgandaQueryService getInstance()
    {
        if(AsyncMtNUgandaQueryService !=null)
        {
            return AsyncMtNUgandaQueryService;
        }
        else
        {
            return new AsyncMtNUgandaQueryService();
        }
    }
        ...
        public String call() throws Exception
        {
            try
            {
                ...
                exec.scheduleAtFixedRate(new Runnable()
                {
                    ...
                    {
                        // do stuff
                        transactionLogger.error("------Inside AsyncMtNUgandaQueryService run() method for trackingid--------" + trackingid);
                ...
                {
                    transactionLogger.error("Async MTNUgandapay awaitTermination---"+trackingid);
                    exec.shutdownNow();
                }
            }
            catch (InterruptedException ie)
            {
                transactionLogger.error("InterruptedException in MTNUgandaPayQuery---",ie);
            }
    ...
    {
        transactionLogger.error("------Inside AsyncMtNUgandaQueryService callQuery method--------");
        ...
        transactionLogger.error("Inside call for AsyncMtNUgandaQueryService---"+trackingid+"--");
        ...
        try
        {
            ...
            if (transactionDetailsVO != null)
            {
                ...
                if (dbStatus.equalsIgnoreCase(PZTransactionStatus.AUTH_STARTED.toString()))
                {
                    ...
                    genericResponseVO = (CommResponseVO) mtnUgandaPaymentGateway.processQuery(trackingid, commRequestVO);
                    ...
                    transactionLogger.error("responseamount ===== " + responseamount);
                    ...
                    if(!compDbAmount.equals(compRsAmount)&&!"Pending".equalsIgnoreCase(transactionStatus)){
                       ...
                       // blacklistVO.setIpAddress(ipAddress);
                        ...
                        blacklistVO.setActionExecutorName("AsyncMtNUgandaQueryService");
...
//                                genericResponseVO.setTransactionType(PZProcessType.THREE_D_SALE.toString());
                                dbBuffer.append("update transaction_common set captureamount='" + amount + "',paymentid='" + transactionId + "',successtimestamp='" + functions.getTimestamp()+ "',status='capturesuccess',remark='" + ESAPI.encoder().encodeForSQL(me, message) + "' where trackingid = " + trackingid);
                        ...
                        else if("failed".equalsIgnoreCase(transactionStatus))
                        {
                            notificationUrl=transactionDetailsVO.getNotificationUrl();
                            status = "failed";
                            dbBuffer.append("update transaction_common set status='authfailed',paymentid='" + transactionId + "',failuretimestamp='" + functions.getTimestamp() + "',remark='" + ESAPI.encoder().encodeForSQL(me, message) + "'");
                        ...
                        if (!"pending".equalsIgnoreCase(status))
                        {
                            ...
                            {
                                transactionLogger.error("inside sending notification from AsyncMTNUgandaQuery---" + notificationUrl);
