 ...
 */
public class AsyncMTNUgandaPayoutQueryService
{
    private static TransactionLogger transactionLogger = new TransactionLogger(AsyncMTNUgandaPayoutQueryService.class.getName());
...
//    public static  AsyncSafexPayQueryService asyncSafexPayQueryService = null;
    public AsyncMTNUgandaPayoutQueryService()
    {
    ...
    public AsyncMTNUgandaPayoutQueryService(String trackingid, String accountId, String paymentId, String afterExecuteTime, String delayTime, String maxExecutionTime)
    {
        //timer = new Timer();
        //timer.schedule(new RemindTask(), seconds*1000);
        transactionLogger.error("inside AsyncMTNUgandaPayoutQueryService -------");
        ...
        public String call() throws Exception
        {
            try
            {
                ...
                exec.scheduleAtFixedRate(new Runnable()
                {
                    ...
                    {
                        // do stuff
                        transactionLogger.error("------Inside AsyncMTNUgandaPayoutQueryService run() method for trackingid--------" + trackingid);
    ...
    {
        transactionLogger.error("------Inside AsyncMTNUgandaPayoutQueryService callQuery method--------");
        ...
        transactionLogger.error("Inside call for AsyncMTNUgandaPayoutQueryService --- "+ trackingid + " --");
...
//        SafexPayPaymentProcess paymentProcess=new SafexPayPaymentProcess();
        ...
        try
        {
            ...
            if (transactionDetailsVO != null)
            {
                ...
                {
                    transRespDetails = (CommResponseVO) mtnUgandaPaymentGateway.processPayoutInquiry(trackingid, commRequestVO);

                    transactionLogger.error("dbstatus for trackingid--->"+dbStatus+"---"+trackingid);
                    transactionLogger.error("genericResponseVO.getTransactionStatus()--->"+transRespDetails.getTransactionStatus());

                    transactionStatus = transRespDetails.getTransactionStatus();
                    ...
                    if (transRespDetails != null && dbStatus.equalsIgnoreCase(PZTransactionStatus.PAYOUT_STARTED.toString()))
                    {
                        int actionEntry1 = 0;
                        ...
                        if ("success".equals(transactionStatus))
                        {
                            ...
                            paymentManager.updateTransactionStatusAfterResponse(String.valueOf(trackingid), ActionEntry.STATUS_PAYOUT_SUCCESSFUL, amount, transRespDetails.getDescription(), transRespDetails.getTransactionId(),null);

                            actionEntry1 = commonPaymentProcess.actionEntry(String.valueOf(trackingid), transRespDetails.getAmount(), ActionEntry.ACTION_PAYOUT_SUCCESSFUL, ActionEntry.STATUS_PAYOUT_SUCCESSFUL, transRespDetails, commRequestVO, auditTrailVO, transRespDetails.getRemark());

                            paymentManager.updateTransactionStatusAfterResponse(String.valueOf(trackingid), ActionEntry.STATUS_PAYOUT_SUCCESSFUL, transRespDetails.getAmount(), ActionEntry.ACTION_PAYOUT_SUCCESSFUL, transRespDetails.getTransactionId(), null);
                        ...
                        else if("fail".equalsIgnoreCase(transactionStatus)||"failed".equalsIgnoreCase(transactionStatus))
                        {
                           ...
                           paymentManager.updateTransactionStatusAfterResponse(String.valueOf(trackingid), ActionEntry.STATUS_PAYOUT_FAILED, amount, transRespDetails.getRemark(), transRespDetails.getTransactionId(),null);

                            actionEntry1 = commonPaymentProcess.actionEntry(String.valueOf(trackingid), amount, ActionEntry.STATUS_PAYOUT_FAILED, ActionEntry.STATUS_PAYOUT_FAILED, transRespDetails, commRequestVO, auditTrailVO, transRespDetails.getRemark());
                        ...
                        if (!"pending".equalsIgnoreCase(transactionStatus))
                        {
                            ...
                            {
                                transactionLogger.error("inside sending notification from AsyncpayoutMTNUgandaQuery---" + notificationUrl);
