 ...
 */
public class AamarPayPaymentProcess extends CommonPaymentProcess
{
    private static TransactionLogger transactionlogger = new TransactionLogger(AamarPayPaymentProcess.class.getName());

    public String get3DConfirmationForm(CommonValidatorVO commonValidatorVO, Comm3DResponseVO response3D){
        transactionlogger.error("inside  AamarPayPaymentProcess Form---");
       ...
       /* Iterator keys=response3D.getRequestMap().keySet().iterator();
        ...
        transactionlogger.error("AamarPayPaymentProcess Form---"+form.toString());
    ...
    {
        transactionlogger.error("inside AirPay Process --->"+directKitResponseVO.getBankRedirectionUrl());
        directKitResponseVO.setBankRedirectionUrl(directKitResponseVO.getBankRedirectionUrl());
        transactionlogger.error("inside AirPay getBankRedirectionUrl Process -->"+directKitResponseVO.getBankRedirectionUrl());
    ...
    public void set3DResponseVO(DirectKitResponseVO directKitResponseVO, Comm3DResponseVO response3D)
    {
        ...
        transactionlogger.debug("inside  AirPay payment process---"+response3D.getUrlFor3DRedirect());
    ...
    {
        transactionlogger.debug("Entering ActionEntry for AamarPayPaymentProcess");
        ...
        try
        {
            ...
            String sql  = "insert into transaction_bangla_details(trackingId,walletname,bankaccount,ifsc,amount,status,dtstamp) values (?,?,?,?,?,?,unix_timestamp(now()))";
        ...
        {
            PZExceptionHandler.raiseDBViolationException(AamarPayPaymentProcess.class.getName(), "actionEntryExtension()", null, "common", "Technical exception", PZDBExceptionEnum.SQL_EXCEPTION, null, se.getMessage(), se.getCause());
        }
        catch (SystemError systemError)
        {
            PZExceptionHandler.raiseDBViolationException(AamarPayPaymentProcess.class.getName(), "actionEntryExtension()", null, "common", "Technical exception", PZDBExceptionEnum.DB_CONNECTION_ISSUE, null, systemError.getMessage(), systemError.getCause());
        }
        finally
        {
