 ...
 */
public class ReproPayUtills
{

    private static TransactionLogger transactionLogger=new TransactionLogger(ReproPayUtills.class.getName());
    ...
    public CommRequestVO getReproPayRequestVO(CommonValidatorVO commonValidatorVO)
    {
...
*/
    ...
    public static String doPostHTTPSURLConnection(String strURL, String request) throws PZTechnicalViolationException
    {
        ...
        try
        {
            ...
            {
                PZExceptionHandler.raiseTechnicalViolationException("ReproPayUtils.java", "doPostHTTPSURLConnection()", null, "common", "SSL Handshake Exception:::", PZTechnicalExceptionEnum.SSL_HANDSHAKE_ISSUE, null, io.getMessage(), io.getCause());
            }
        ...
        {
            PZExceptionHandler.raiseTechnicalViolationException("ReproPayUtils.java", "doPostHTTPSURLConnection()", null, "common", "Technical Exception Occurred:::", PZTechnicalExceptionEnum.UNSUPPORTED_ENCOADING_EXCEPTION, null, ue.getMessage(), ue.getCause());
        }
        catch (MalformedURLException me)
        {
            PZExceptionHandler.raiseTechnicalViolationException("ReproPayUtils.java", "doPostHTTPSURLConnection()", null, "common", "Technical Exception Occurred:::", PZTechnicalExceptionEnum.MALFORMED_URL_EXCEPITON, null, me.getMessage(), me.getCause());
        }
        catch (ProtocolException pe)
        {
            PZExceptionHandler.raiseTechnicalViolationException("ReproPayUtils.java", "doPostHTTPSURLConnection()", null, "common", "Technical Exception Occurred:::", PZTechnicalExceptionEnum.PROTOCOL_EXCEPTION, null, pe.getMessage(), pe.getCause());
        }
        catch (IOException ex)
        {
            PZExceptionHandler.raiseTechnicalViolationException("ReproPayUtils.java", "doPostHTTPSURLConnection()", null, "common", "Technical Exception Occurred:::", PZTechnicalExceptionEnum.IOEXCEPTION, null, ex.getMessage(), ex.getCause());
        }
    ...
    {
        LinkedList<Repropay> bufList = new LinkedList<Repropay>();
        int size = 0;
        String buffer = null;
        byte buf[];
        try
        {
            do
            {
                ...
                if (num == -1)
                    break;
                size += num;
                bufList.add(new Repropay(buf, num));
            }
            while (true);
            buf = new byte[size];
            int pos = 0;
            for (ListIterator<Repropay> p = bufList.listIterator(); p.hasNext(); )
            {
                Repropay b = p.next();
    ...
    static class Repropay
    {
        public byte buf[];
        public int size;

        public Repropay(byte b[], int s)
        {
            buf = b;
            size = s;
        }
    ...
    public HashMap getPreviousTransactionDeatils(String trackingid){
        ...
        ResultSet rs= null;
        try{
            String query="Select * from transaction_Repropay_details where trackingid='"+trackingid+"'";
    ...
    public static String getResponseForm(HashMap<String, String> parameters,String url){
        ...
        System.out.println("Repropay PaymentProcess Form---"+form.toString());
        return form.toString();
    }


    public void insertRepropayDetails(String Trackingid,String Ds_Response,String Ds_TransactionType,String Ds_Terminal,String Ds_Date,String Ds_AuthorisationCode){
        Connection conn=null;
        PreparedStatement ps=null;
        try{
            conn=Database.getConnection();
            String query="insert into transaction_Repropay_details(Trackingid,Ds_Response,Ds_TransactionType,Ds_Terminal,Ds_Date,Ds_AuthorisationCode) values(?,?,?,?,?,?)";
            ps=conn.prepareStatement(query);
    ...
    public void updateRepropayDetails(String Trackingid,String Ds_Response,String Ds_TransactionType,String Ds_Terminal,String Ds_Date,String Ds_AuthorisationCode){
        Connection conn=null;
        PreparedStatement ps=null;
        try{
            conn=Database.getConnection();
            String query="UPDATE transaction_Repropay_details set Ds_Response=?,Ds_TransactionType=?,Ds_Terminal=?,Ds_Date=?,Ds_AuthorisationCode=? where Trackingid=? ";
            ps=conn.prepareStatement(query);
