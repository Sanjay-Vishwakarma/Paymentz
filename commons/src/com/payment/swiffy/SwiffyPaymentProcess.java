 ...
 */
public class SwiffyPaymentProcess extends CommonPaymentProcess
{
  private static TransactionLogger transactionLogger=new TransactionLogger(SwiffyPaymentProcess.class.getName());
    public String get3DConfirmationForm(CommonValidatorVO CommonValidatorVO,Comm3DResponseVO response3d)
    {
      transactionLogger .error("inside SwiffyPaymentProcess From----");
      ...
      transactionLogger.error(CommonValidatorVO.getTrackingid() + "SwiffyPaymentProcess" + form.toString());
     ...
     {
       transactionLogger.error("inside SwiffyPayment getBankRedirectionUrl Process -->"+directKitResponseVO.getBankRedirectionUrl());
       return directKitResponseVO;
     }
     public void set3DResponseVO(DirectKitResponseVO directKitResponseVO, Comm3DResponseVO response3D)
     {
       transactionLogger.error("inside  SwiffyPaymentProcess---" + response3D.getUrlFor3DRedirect());
      ...
      {
        transactionLogger.error("Entering ActionEntry for SwiffyPaymentProcess");
        int results=0;
        Connection cn = null;
        try
        {
        ...
        {
          PZExceptionHandler.raiseDBViolationException(SwiffyPaymentProcess.class.getName(), "actionEntryExtension()", null, "common", "Technical exception", PZDBExceptionEnum.SQL_EXCEPTION, null, se.getMessage(), se.getCause());
        }
        catch (SystemError systemError)
        {
          PZExceptionHandler.raiseDBViolationException(SwiffyPaymentProcess.class.getName(), "actionEntryExtension()", null, "common", "Technical exception", PZDBExceptionEnum.DB_CONNECTION_ISSUE, null, systemError.getMessage(), systemError.getCause());
        }
        finally
        {
