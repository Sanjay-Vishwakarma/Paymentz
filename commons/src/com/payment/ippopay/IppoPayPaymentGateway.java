...
public class IppoPayPaymentGateway  extends AbstractPaymentGateway
{
    ...
    private static IppoPayTransactionLogger transactionlogger = new IppoPayTransactionLogger(IppoPayPaymentGateway.class.getName());
   ...
   /* public GenericResponseVO process3DSaleConfirmation(String trackingID, GenericRequestVO requestVO)
    ...
    public GenericResponseVO processRefund(String trackingID, GenericRequestVO requestVO) throws PZTechnicalViolationException, PZConstraintViolationException, PZDBViolationException
    {
        ...
        transactionlogger.error("paymentid is-------------->"+paymentid);
        String public_key =gatewayAccount.getFRAUD_FTP_PASSWORD();
        transactionlogger.error("public_key is-------------->" + public_key);
        String secret_key =gatewayAccount.getFRAUD_FTP_PATH();
        transactionlogger.error("secret_key is-------------->"+secret_key);
        ...
        try
        {
            ...
            if (isTest)
            {
                transactionlogger.error("inside ippoPay Test Refund --->");
                refundResponse = ippoPayUtils.doGetHttpUrlConnection2(RB.getString("REFUND_LIVE")+paymentid, "",public_key,secret_key);
            }
            else
            {
                transactionlogger.error("inside ippoPay Live Refund --->");
                refundResponse = ippoPayUtils.doGetHttpUrlConnection2(RB.getString("REFUND_LIVE")+paymentid,  "",public_key,secret_key);
            }
            transactionlogger.error("refund url-----" + RB.getString("REFUND_LIVE"));
            ...
            if (functions.isValueNull(refundResponse) && refundResponse.contains("{"))
            {
                ...
                if (jsonObject1 != null)
                {
                    ...
                    {
                        transactionlogger.error("inside success -----" + code);
                 ...
                 //   commResponseVO.setCurrency(currency);
                  //  commResponseVO.setErrorCode();
                    commResponseVO.setTransactionType(success);
                }
            }
            else
            {
        ...
        {
            PZExceptionHandler.raiseTechnicalViolationException(IppoPayPaymentGateway.class.getName(), "processSale()", null, "common", "Technical Exception while placing the transaction", PZTechnicalExceptionEnum.JSON_EXCEPTION, null, e.getMessage(), e.getCause());
        }
        return commResponseVO;
    }
