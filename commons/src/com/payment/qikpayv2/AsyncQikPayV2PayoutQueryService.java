 ...
 */
public class AsyncQikPayV2PayoutQueryService
{

    private static TransactionLogger transactionLogger  = new TransactionLogger(AsyncQikPayV2PayoutQueryService.class.getName());
    ...
    public static AsyncQikPayV2PayoutQueryService asyncQikPayV2PayoutQueryService = null;
    public AsyncQikPayV2PayoutQueryService()
    {
    ...
    public AsyncQikPayV2PayoutQueryService(String trackingid, String accountId, String paymentId, String afterExecuteTime, String delayTime, String maxExecutionTime)
    {
        //timer = new Timer();
        //timer.schedule(new RemindTask(), seconds*1000);
        transactionLogger.error("inside asyncQikPayV2PayoutQueryService -------");

        callAsyncApexPayQuery(trackingid, accountId, paymentId, afterExecuteTime, delayTime, maxExecutionTime);
    ...
    public static AsyncQikPayV2PayoutQueryService getInstance()
    {
        if(asyncQikPayV2PayoutQueryService !=null)
        {
            return asyncQikPayV2PayoutQueryService;
        }
        else
        {
            return new AsyncQikPayV2PayoutQueryService();
        }
    }


    public Future<String> callAsyncApexPayQuery(String trackingid,String accountId,String paymentId,String afterExecuteTime,String delayTime,String maxExecutionTime)
    {
        return executorService.submit(new QikPayV2QueryCallable(trackingid,accountId,paymentId, afterExecuteTime, delayTime, maxExecutionTime));
    }
    public class QikPayV2QueryCallable implements Callable<String>
    {
        ...
        private QikPayV2QueryCallable(String trackingid,String accountId,String paymentId,String afterExecuteTime,String delayTime,String maxExecutionTime)
        {
            this.paymentId          = paymentId;
            this.trackingid         = trackingid;
            this.accountId          = accountId;
        ...
        public String call() throws Exception
        {
            try
            {
                ...
                exec.scheduleAtFixedRate(new Runnable()
                {
                    ...
                    {
                        // do stuff
                        transactionLogger.error("------Inside asyncQikPayV2PayoutQueryService run() method for trackingid--------" + trackingid);
                ...
                {
                    transactionLogger.error("Async ApexPay awaitTermination---"+trackingid);
                    exec.shutdownNow();
                }
            }
            catch (InterruptedException ie)
            {
                transactionLogger.error("InterruptedException in ApexPayQuery---",ie);
            }
    ...
    {
        transactionLogger.error("------Inside asyncQikPayV2PayoutQueryService callQuery method--------trackingid "+trackingid+" paymentId--->"+paymentId);
        Functions functions     = new Functions();
        QikPayV2PaymentGateway qikPayV2PaymentGateway = new QikPayV2PaymentGateway(accountId);
        ...
        try
        {
            ...
            if (transactionDetailsVO != null)
            {
                ...
                if (dbStatus.equalsIgnoreCase(PZTransactionStatus.PAYOUT_STARTED.toString()))
                {
                    ...
                    genericResponseVO = (CommResponseVO) qikPayV2PaymentGateway.processPayoutInquiry(trackingid, commRequestVO);
                    ...
                    if (genericResponseVO != null && dbStatus.equalsIgnoreCase(PZTransactionStatus.PAYOUT_STARTED.toString()))
                    {
                        ...
                        else if("fail".equalsIgnoreCase(transactionStatus)||"failed".equalsIgnoreCase(transactionStatus))
                        {
                            ...
                            {
                                transactionLogger.error("inside sending notification from AsyncpayoutQikPayV2Query---" + notificationUrl);
