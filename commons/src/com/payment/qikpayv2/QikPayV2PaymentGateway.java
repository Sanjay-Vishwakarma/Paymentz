 ...
 */
public class QikPayV2PaymentGateway extends AbstractPaymentGateway
{
    private static TransactionLogger transactionlogger  = new TransactionLogger(QikPayV2PaymentGateway.class.getName());
    public static final String GATEWAY_TYPE             = "qikpayv2";
    private final static ResourceBundle RB              = LoadProperties.getProperty("com.directi.pg.qikpayv2");
    private final static ResourceBundle RB_NB           = LoadProperties.getProperty("com.directi.pg.QPV2BANKS");
    ...
    public QikPayV2PaymentGateway(String accountId)
    {
    ...
    {
        transactionlogger.error("Entering processSale of QikPayV2PaymentGateway......");
        ...
        String RETURN_URL  = RB.getString("QIKPAY_RU")+trackingID;
        String REQUEST_URL = "";
        ...
        String  PAYMENT_TYPE   = QikPayV2Utils.getPaymentType(payment_Card);//CARD/WL/NB
        String  MOP_TYPE       = "";
        String  AMOUNT         = QikPayV2Utils.getAmount(transactionDetailsVO.getAmount());
        ...
        try
        {
            ...
            if("CARD".equalsIgnoreCase(PAYMENT_TYPE))
            {
                ...
                MOP_TYPE = QikPayV2Utils.getPaymentBrand(payment_brand);
            }
            ...
            HASH                    = QikPayV2Utils.generateCheckSum(requestMap, SALT);
            String requestString    = QikPayV2Utils.generateCheckSum2(requestMap,HASH);
            transactionlogger.error("final requestString ---> "+trackingID+" "+requestString);
            ENCDATA                 = QikPayV2Utils.encryptAES(requestString,SECRETE_KEY);
            ...
            transactionlogger.error("QikPayV2PaymentGateway ---------------> ",e);
        }
    ...
    public String  processAutoRedirect(CommonValidatorVO commonValidatorVO) throws PZTechnicalViolationException, PZDBViolationException
    {
        ...
        QikPayV2Utils QikPayV2Utils                         = new QikPayV2Utils();
        QikPayV2PaymentProcess apexPayPaymentProcess = new QikPayV2PaymentProcess();
        CommRequestVO commRequestVO     = QikPayV2Utils.getQPayNRequestVO(commonValidatorVO);
        GatewayAccount gatewayAccount   = GatewayAccountService.getGatewayAccount(accountId);
        ...
        {
            transactionlogger.error("PZGenericConstraintViolationException in QikPayV2PaymentGateway---", e);
        }
        return html;
    }
    ...
    {
        transactionlogger.error("Inside processInquiry of QikPay---");
        ...
        String SALT             = gatewayAccount.getFRAUD_FTP_USERNAME();
        String PAY_ID           = gatewayAccount.getMerchantId();
        String AMOUNT           = QikPayV2Utils.getAmount(commTransactionDetailsVO.getAmount());
        ...
        try
        {
            ...
            HASH = QikPayV2Utils.generateCheckSum(hashMap, SALT);
            ...
            inquiry_res = QikPayV2Utils.doGetHTTPSURLConnectionClient(REQUEST_URL,parameters.toString());
            ...
            if (functions.isValueNull(inquiry_res) && inquiry_res.contains("{"))
            {
                ...
                if (jsonObject != null)
                {
                    ...
                    else if(responseCode.equalsIgnoreCase("Pending") || responseCode.equalsIgnoreCase("Enrolled") || responseCode.equalsIgnoreCase("Sent to Bank") )
                    {
                    ...
                    else if(status.equalsIgnoreCase("Invalid") || status.equalsIgnoreCase("Declined") || status.equalsIgnoreCase("Rejected")|| status.equalsIgnoreCase("Error")
                            ||"Failed".equalsIgnoreCase(status) ||"Cancelled".equalsIgnoreCase(status))
                    {
        ...
        {
            transactionlogger.error("QikPayV2PaymentGateway processQuery JSONException--->",e);
        }catch (Exception e){
            transactionlogger.error("QikPayV2PaymentGateway processQuery Exception--->",e);
        }
        return commResponseVO;
    }
    ...
    public GenericResponseVO processPayout(String trackingId,GenericRequestVO requestVO) throws PZConstraintViolationException, PZTechnicalViolationException, PZGenericConstraintViolationException
    {
        ...
        try
        {
            ...
            HASH = QikPayV2Utils.getPayOutHash(strForHash);
            ...
            transactionlogger.error("processPayoutRequest strForHash------> "+strForHash);
            transactionlogger.error("processPayoutRequest ------> "+trackingId +" "+jsonObject.toString());

            String responeString = QikPayV2Utils.doGetHTTPSURLConnectionClient(hostURL, jsonObject.toString());

            transactionlogger.error("processPayoutResponse ------> "+trackingId+" "+responeString);
            ...
            String txtStatus         = "";
                ...
                if(functions.isValueNull(responeString) && responeString.contains("{"))
                {
                    ...
                    if (txtStatus.equalsIgnoreCase("SUCCESS"))
                    {
                        ...
                        QikPayV2Utils.updateRRNMainTableEntry(RRN,trackingId);
                    }
    ...
    {
        transactionlogger.error("-----inside QikPay processPayout inquiry------->");
    ...
    public GenericResponseVO processAuthentication(String trackingID, GenericRequestVO requestVO) throws PZGenericConstraintViolationException
    {
        ...
        PZGenericConstraint genConstraint = new PZGenericConstraint("QikPayV2PaymentGateway","processAuthentication",null,"common","This Functionality is not supported by processing gateway. Please contact your Tech. support Team:::",errorCodeListVO);
