 ...
 */
public class AsyncQikPayPayoutQueryService
{

    private static QikPayGatewayLogger transactionLogger = new QikPayGatewayLogger(AsyncQikPayPayoutQueryService.class.getName());
    ...
    public static AsyncQikPayPayoutQueryService asyncQikPayPayoutQueryService = null;
    public   AsyncQikPayPayoutQueryService()
    {
    ...
    public AsyncQikPayPayoutQueryService(String trackingid,String accountId,String paymentId,String afterExecuteTime,String delayTime,String maxExecutionTime)
    {
        //timer = new Timer();
        //timer.schedule(new RemindTask(), seconds*1000);
        transactionLogger.error("inside AsyncQikPayPayoutQueryService -------");
    ...
    public static AsyncQikPayPayoutQueryService getInstance()
    {
        if(asyncQikPayPayoutQueryService !=null)
        {
            return asyncQikPayPayoutQueryService;
        }
        else
        {
            return new AsyncQikPayPayoutQueryService();
        }
    }
    ...
    {
        return executorService.submit(new QikPayQueryCallable(trackingid,accountId,paymentId, afterExecuteTime, delayTime, maxExecutionTime));
    }
    public class QikPayQueryCallable implements Callable<String>
    {
        ...
        private QikPayQueryCallable(String trackingid,String accountId,String paymentId,String afterExecuteTime,String delayTime,String maxExecutionTime)
        {
            this.paymentId = paymentId;
            this.trackingid = trackingid;
            this.accountId = accountId;
        ...
        public String call() throws Exception
        {
            try
            {
                ...
                exec.scheduleAtFixedRate(new Runnable()
                {
                    ...
                    {
                        // do stuff
                        transactionLogger.error("Inside AsyncQikPayPayoutQueryService run() method for trackingid--------" + trackingid);
                ...
                {
                    transactionLogger.error("Async QikPay inside if maxExecutionTime awaitTermination---"+trackingid);
                    exec.shutdownNow();
                }
            }
            catch (InterruptedException ie)
            {
                transactionLogger.error("InterruptedException in QikPayQuery---",ie);
            }
    ...
    {
        transactionLogger.error("------Inside AsyncQikPayPayoutQueryService callQuery method--------");
        ...
        transactionLogger.error("Inside callQuery for AsyncQikPayPayoutQueryService---"+trackingid+"--");
        ...
        try
        {
            ...
            if (transactionDetailsVO != null)
            {
                ...
                dbStatus = transactionDetailsVO.getStatus();
                transactionLogger.debug("AsyncQikpay dbStatus-----" + dbStatus);
                if (dbStatus.equalsIgnoreCase(PZTransactionStatus.PAYOUT_STARTED.toString()))
                {
                    ...
                    genericResponseVO = (CommResponseVO) qikpayPaymentGateway.processPayoutInquiry(trackingid, commRequestVO);

                    transactionLogger.error("inside if payout started AsyncQikpay dbstatus for trackingid--->"+dbStatus+"---"+trackingid);
                    ...
                    if (genericResponseVO != null && dbStatus.equalsIgnoreCase(PZTransactionStatus.PAYOUT_STARTED.toString()))
                    {
                        ...
                        if (!"pending".equalsIgnoreCase(transactionStatus))
                        {
                            ...
                            {
                                transactionLogger.error("inside sending notification from AsyncpayoutQikpayQuery---" + notificationUrl);
