 ...
 */
public class AsyncBhartiPayPayoutService
{

    private static TransactionLogger transactionLogger = new TransactionLogger(AsyncBhartiPayPayoutService.class.getName());
    ...
    public static AsyncBhartiPayPayoutService asyncBhartiPayPayoutService = null;
    public   AsyncBhartiPayPayoutService()
    {
    ...
    public AsyncBhartiPayPayoutService(String trackingid,String accountId,String paymentId,String afterExecuteTime,String delayTime,String maxExecutionTime)
    {
        //timer = new Timer();
        //timer.schedule(new RemindTask(), seconds*1000);
        transactionLogger.error("inside AsyncBhartiPayPayoutService -------");

        callAsyncBhartiPayQuery(trackingid, accountId, paymentId, afterExecuteTime, delayTime, maxExecutionTime);
    ...
    public static AsyncBhartiPayPayoutService getInstance()
    {
        if(asyncBhartiPayPayoutService !=null)
        {
            return asyncBhartiPayPayoutService;
        }
        else
        {
            return new AsyncBhartiPayPayoutService();
        }
    }


    public Future<String> callAsyncBhartiPayQuery(String trackingid,String accountId,String paymentId,String afterExecuteTime,String delayTime,String maxExecutionTime)
    {
        return executorService.submit(new BhartiPayQueryCallable(trackingid,accountId,paymentId, afterExecuteTime, delayTime, maxExecutionTime));
    }
    public class BhartiPayQueryCallable implements Callable<String>
    {
        ...
        private BhartiPayQueryCallable(String trackingid,String accountId,String paymentId,String afterExecuteTime,String delayTime,String maxExecutionTime)
        {
            this.paymentId = paymentId;
            this.trackingid = trackingid;
            this.accountId = accountId;
        ...
        public String call() throws Exception
        {
            try
            {
                ...
                exec.scheduleAtFixedRate(new Runnable()
                {
                    ...
                    {
                        // do stuff
                        transactionLogger.error("------Inside AsyncBhartiPayPayoutQueryService run() method for trackingid--------" + trackingid);
                ...
                {
                    transactionLogger.error("Async BhartiPay awaitTermination---"+trackingid);
                    exec.shutdownNow();
                }
            }
            catch (InterruptedException ie)
            {
                transactionLogger.error("InterruptedException in BhartiPayQuery---",ie);
            }
    ...
    {
        transactionLogger.error("------Inside AsyncBhartiPayPayoutQueryService callQuery method--------");
        ...
        transactionLogger.error("Inside call for AsyncBhartiPayPayoutQueryService---"+trackingid+"--");
        ...
        try
        {
            ...
            if (transactionDetailsVO != null)
            {
                ...
                auditTrailVO.setActionExecutorName("AsyncBhartiPayPayoutQueryService");
                ...
                if (dbStatus.equalsIgnoreCase(PZTransactionStatus.PAYOUT_STARTED.toString()))
                {
                    ...
                    genericResponseVO = (CommResponseVO) bhartiPayPaymentGateway.processPayoutInquiry(trackingid, commRequestVO);
                    ...
                    if (genericResponseVO != null && dbStatus.equalsIgnoreCase(PZTransactionStatus.PAYOUT_STARTED.toString()))
                    {
                        ...
                        if (!"pending".equalsIgnoreCase(transactionStatus))
                        {
                            ...
                            {
                                transactionLogger.error("inside sending notification from AsyncpayoutBhartipayQuery---" + notificationUrl);
