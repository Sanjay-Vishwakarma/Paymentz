 ...
 */
public class CashFreePaymentProcess extends CommonPaymentProcess
{
    private static TransactionLogger transactionlogger = new TransactionLogger(CashFreePaymentProcess.class.getName());

    public String get3DConfirmationForm(CommonValidatorVO commonValidatorVO, Comm3DResponseVO response3D){
        transactionlogger.error("inside  CashFreePaymentProcess Form---");
        ...
        form+="<input type=\"hidden\" name=\"signature\"  value=\""+response3D.getPaReq()+"\">";
        form+="</form><script language=\"javascript\">document.creditcard_checkout.submit();</script>";

        transactionlogger.error("CashFreePaymentProcess Form---"+form.toString());
    ...
    {
        transactionlogger.error("inside CashFree Process --->"+directKitResponseVO.getBankRedirectionUrl());
        directKitResponseVO.setBankRedirectionUrl(directKitResponseVO.getBankRedirectionUrl());
        transactionlogger.error("inside CashFree getBankRedirectionUrl Process -->"+directKitResponseVO.getBankRedirectionUrl());
