...
public class EPaySolutionGateway extends AbstractPaymentGateway
{
    ...
    public GenericResponseVO processSale(String trackingId, GenericRequestVO requestVO) throws PZTechnicalViolationException
    {
        ...
        try
        {
            ...
            String requestForAuthlog = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\n" +
                    "<merNo>" + merchantAccountNo + "</merNo>\n" +
                    "<gatewayNo>" + gatewayNo + "</gatewayNo>\n" +
                    "<orderNo>" + orderId + "</orderNo>\n" +
                    "<orderCurrency>" + currency + "</orderCurrency>\n" +
                    "<orderAmount>" + amount + "</orderAmount>\n" +
                    "<cardNo>" + functions.maskingPan(cardNumber) + "</cardNo>\n" +
                    "<cardExpireMonth>" + functions.maskingNumber(expMonth) + "</cardExpireMonth>\n" +
                    "<cardExpireYear>" + functions.maskingNumber(expYear) + "</cardExpireYear>\n" +
                    "<cCode>" + functions.maskingNumber(cvv) + "</cCode>\n" +
                    "<issuingBank>" + "IssuingBank" + "</issuingBank>\n" +
                    "<firstName>" + firstName + "</firstName>\n" +
                    "<lastName>" + lastName + "</lastName>\n" +
                    "<email>" + email + "</email>\n" +
            ...
            transactionLogger.error("sale request-----" + trackingId + "--" + requestForAuthlog);
            ...
            transactionLogger.error("sale response with tradedata-----" + trackingId + "--" + requestWithTrade);
