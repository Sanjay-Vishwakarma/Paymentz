 ...
 */
public class AsyncBnmQuickPayoutQueryService
{

    private static TransactionLogger transactionLogger  = new TransactionLogger(AsyncBnmQuickPayoutQueryService.class.getName());
    ...
    public static AsyncBnmQuickPayoutQueryService asyncBnmQuickPayoutQueryService = null;
    public AsyncBnmQuickPayoutQueryService()
    {
    ...
    public AsyncBnmQuickPayoutQueryService(String trackingid, String accountId, String paymentId, String afterExecuteTime, String delayTime, String maxExecutionTime)
    {
        //timer = new Timer();
        //timer.schedule(new RemindTask(), seconds*1000);
        transactionLogger.error("inside AsyncBnmQuickPayoutQueryService -------");
    ...
    public static AsyncBnmQuickPayoutQueryService getInstance()
    {
        if(asyncBnmQuickPayoutQueryService !=null)
        {
            return asyncBnmQuickPayoutQueryService;
        }
        else
        {
            return new AsyncBnmQuickPayoutQueryService();
        }
    }
    ...
    {
        return executorService.submit(new BNMQueryCallable(trackingid,accountId,paymentId, afterExecuteTime, delayTime, maxExecutionTime));
    }
    public class BNMQueryCallable implements Callable<String>
    {
        ...
        private BNMQueryCallable(String trackingid,String accountId,String paymentId,String afterExecuteTime,String delayTime,String maxExecutionTime)
        {
            this.paymentId          = paymentId;
            this.trackingid         = trackingid;
            this.accountId          = accountId;
        ...
        public String call() throws Exception
        {
            try
            {
                ...
                exec.scheduleAtFixedRate(new Runnable()
                {
                    ...
                    {
                        // do stuff
                        transactionLogger.error("------Inside AsyncBnmQuickPayoutQueryService run() method for trackingid--------" + trackingid);
                ...
                {
                    transactionLogger.error("Async Bnmquick awaitTermination---"+trackingid);
                    exec.shutdownNow();
                }
            }
            catch (InterruptedException ie)
            {
                transactionLogger.error("InterruptedException in BnmquickQuery---",ie);
            }
    ...
    {
        transactionLogger.error("------Inside AsyncBnmQuickPayoutQueryService callQuery method--------trackingid "+trackingid+" paymentId--->"+paymentId);
        Functions functions     = new Functions();
        BnmQuickPaymentGateway bnmQuickPaymentGateway   = new BnmQuickPaymentGateway(accountId);
        ...
        try
        {
            ...
            if (transactionDetailsVO != null)
            {
                ...
                if (dbStatus.equalsIgnoreCase(PZTransactionStatus.PAYOUT_STARTED.toString()))
                {
                    ...
                    genericResponseVO = (CommResponseVO) bnmQuickPaymentGateway.processPayoutInquiry(trackingid, commRequestVO);
