 ...
 */
public class AsyncApexPayPayoutQueryService
{

    private static TransactionLogger transactionLogger  = new TransactionLogger(AsyncApexPayPayoutQueryService.class.getName());
    ...
    public static AsyncApexPayPayoutQueryService asyncApexPayPayoutQueryService = null;
    public AsyncApexPayPayoutQueryService()
    {
    ...
    public AsyncApexPayPayoutQueryService(String trackingid, String accountId, String paymentId, String afterExecuteTime, String delayTime, String maxExecutionTime)
    {
        //timer = new Timer();
        //timer.schedule(new RemindTask(), seconds*1000);
        transactionLogger.error("inside AsyncApexPayPayoutQueryService -------");

        callAsyncIMonePayQuery(trackingid, accountId, paymentId, afterExecuteTime, delayTime, maxExecutionTime);
    ...
    public static AsyncApexPayPayoutQueryService getInstance()
    {
        if(asyncApexPayPayoutQueryService !=null)
        {
            return asyncApexPayPayoutQueryService;
        }
        else
        {
            return new AsyncApexPayPayoutQueryService();
        }
    }


    public Future<String> callAsyncIMonePayQuery(String trackingid,String accountId,String paymentId,String afterExecuteTime,String delayTime,String maxExecutionTime)
    {
        return executorService.submit(new ApexPayQueryCallable(trackingid,accountId,paymentId, afterExecuteTime, delayTime, maxExecutionTime));
    }
    public class ApexPayQueryCallable implements Callable<String>
    {
        ...
        private ApexPayQueryCallable(String trackingid,String accountId,String paymentId,String afterExecuteTime,String delayTime,String maxExecutionTime)
        {
            this.paymentId          = paymentId;
            this.trackingid         = trackingid;
            this.accountId          = accountId;
        ...
        public String call() throws Exception
        {
            try
            {
                ...
                exec.scheduleAtFixedRate(new Runnable()
                {
                    ...
                    {
                        // do stuff
                        transactionLogger.error("------Inside AsyncApexPayPayoutQueryService run() method for trackingid--------" + trackingid);
            ...
            {
                transactionLogger.error("InterruptedException in apexPayPayQuery---",ie);
            }
    ...
    {
        transactionLogger.error("------Inside AsyncApexPayPayoutQueryService callQuery method--------trackingid "+trackingid+" paymentId--->"+paymentId);
        Functions functions     = new Functions();
        ApexPayPaymentGateway apexPayPaymentGateway   = new ApexPayPaymentGateway(accountId);
        ...
        try
        {
            ...
            if (transactionDetailsVO != null)
            {
                ...
                if (dbStatus.equalsIgnoreCase(PZTransactionStatus.PAYOUT_STARTED.toString()))
                {
                    ...
                    genericResponseVO = (CommResponseVO) apexPayPaymentGateway.processPayoutInquiry(trackingid, commRequestVO);
                    ...
                    if (genericResponseVO != null && dbStatus.equalsIgnoreCase(PZTransactionStatus.PAYOUT_STARTED.toString()))
                    {
                        ...
                        {
                            transactionLogger.error("inside sending notification from AsyncApexPayPayoutQueryService---" + notificationUrl);
