...
import com.directi.pg.PayGTransactionLogger;
 ...
 */
public class PayGPaymentProcess extends CommonPaymentProcess
{
    //private static TransactionLogger transactionlogger = new TransactionLogger(PayGPaymentProcess.class.getName());
    private static PayGTransactionLogger transactionlogger = new PayGTransactionLogger(PayGPaymentProcess.class.getName());

    public String get3DConfirmationForm(CommonValidatorVO commonValidatorVO, Comm3DResponseVO response3D){
        transactionlogger.error("inside  PayGPaymentProcess Form ---");
        ...
        transactionlogger.error("PayGPaymentProcess Form---"+form.toString());
    ...
    public void set3DResponseVO(DirectKitResponseVO directKitResponseVO, Comm3DResponseVO response3D, CommonValidatorVO commonValidatorVO)
    {
        ...
        transactionlogger.debug("inside  PayG payment process---"+response3D.getUrlFor3DRedirect());
    ...
    public int actionEntryExtension(int newDetailId, String trackingId, String amount, String action, String status, CommResponseVO responseVO, CommRequestVO commRequestVO) throws PZDBViolationException
    {
        ...
        {
            PZExceptionHandler.raiseDBViolationException(PayGPaymentGateway.class.getName(), "actionEntryExtension()", null, "common", "Technical exception", PZDBExceptionEnum.SQL_EXCEPTION, null, se.getMessage(), se.getCause());
        }
        catch (SystemError systemError)
        {
            PZExceptionHandler.raiseDBViolationException(PayGPaymentGateway.class.getName(), "actionEntryExtension()", null, "common", "Technical exception", PZDBExceptionEnum.DB_CONNECTION_ISSUE, null, systemError.getMessage(), systemError.getCause());
        }
        finally
        {
