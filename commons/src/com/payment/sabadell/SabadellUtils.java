 ...
 */
public class SabadellUtils
{
    private final static String charset = "UTF-8";

    private static TransactionLogger transactionLogger= new TransactionLogger(SabadellUtils.class.getName());
    ...
    public static String doPostHTTPSURLConnection(String strURL, String request) throws PZTechnicalViolationException
    {
        ...
        try
        {
            ...
            {
                PZExceptionHandler.raiseTechnicalViolationException("SabadellUtils.java", "doPostHTTPSURLConnection()", null, "common", "SSL Handshake Exception:::", PZTechnicalExceptionEnum.SSL_HANDSHAKE_ISSUE, null, io.getMessage(), io.getCause());
            }
        ...
        {
            PZExceptionHandler.raiseTechnicalViolationException("SabadellUtils.java", "doPostHTTPSURLConnection()", null, "common", "Technical Exception Occurred:::", PZTechnicalExceptionEnum.UNSUPPORTED_ENCOADING_EXCEPTION, null, ue.getMessage(), ue.getCause());
        }
        catch (MalformedURLException me)
        {
            PZExceptionHandler.raiseTechnicalViolationException("SabadellUtils.java", "doPostHTTPSURLConnection()", null, "common", "Technical Exception Occurred:::", PZTechnicalExceptionEnum.MALFORMED_URL_EXCEPITON, null, me.getMessage(), me.getCause());
        }
        catch (ProtocolException pe)
        {
            PZExceptionHandler.raiseTechnicalViolationException("SabadellUtils.java", "doPostHTTPSURLConnection()", null, "common", "Technical Exception Occurred:::", PZTechnicalExceptionEnum.PROTOCOL_EXCEPTION, null, pe.getMessage(), pe.getCause());
        }
        catch (IOException ex)
        {
            PZExceptionHandler.raiseTechnicalViolationException("SabadellUtils.java", "doPostHTTPSURLConnection()", null, "common", "Technical Exception Occurred:::", PZTechnicalExceptionEnum.IOEXCEPTION, null, ex.getMessage(), ex.getCause());
        }
    ...
    {
        LinkedList<Sabadell> bufList = new LinkedList<Sabadell>();
        int size = 0;
        String buffer = null;
        byte buf[];
        try
        {
            do
            {
                ...
                if (num == -1)
                    break;
                size += num;
                bufList.add(new Sabadell(buf, num));
            }
            while (true);
            buf = new byte[size];
            int pos = 0;
            for (ListIterator<Sabadell> p = bufList.listIterator(); p.hasNext(); )
            {
                Sabadell b = p.next();
    ...
    static class Sabadell
    {
        public byte buf[];
        public int size;

        public Sabadell(byte b[], int s)
        {
            buf = b;
            size = s;
        }
    ...
    public HashMap  getPreviousTransactionDeatils(String trackingid){
        ...
        ResultSet rs= null;
        try{
            String query="Select * from transaction_sabadell_details where trackingid='"+trackingid+"'";
    ...
    public void insertSabadellDetails(String Trackingid,String Ds_Response,String Ds_TransactionType,String Ds_Terminal,String Ds_Date,String Ds_AuthorisationCode){
        Connection conn=null;
        PreparedStatement ps=null;
        try{
            conn=Database.getConnection();
            String query="insert into transaction_sabadell_details(Trackingid,Ds_Response,Ds_TransactionType,Ds_Terminal,Ds_Date,Ds_AuthorisationCode) values(?,?,?,?,?,?)";
            ps=conn.prepareStatement(query);
    ...
    public void updateSabadellDetails(String Trackingid,String Ds_Response,String Ds_TransactionType,String Ds_Terminal,String Ds_Date,String Ds_AuthorisationCode){
        Connection conn=null;
        PreparedStatement ps=null;
        try{
            conn=Database.getConnection();
            String query="UPDATE transaction_sabadell_details set Ds_Response=?,Ds_TransactionType=?,Ds_Terminal=?,Ds_Date=?,Ds_AuthorisationCode=? where Trackingid=? ";
            ps=conn.prepareStatement(query);
