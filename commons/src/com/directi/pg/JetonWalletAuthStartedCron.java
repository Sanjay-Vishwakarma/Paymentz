 ...
 */
public class JetonWalletAuthStartedCron
{
    private static Logger logger = new Logger(JetonWalletAuthStartedCron.class.getName());
    private static TransactionLogger transactionLogger = new TransactionLogger(JetonWalletAuthStartedCron.class.getName());
    boolean isLogEnabled= Boolean.parseBoolean(ApplicationProperties.getProperty("IS_LOG_ENABLED"));


    public String jetonWalletAuthStartedCron(Hashtable hashtable)
    {
        if(isLogEnabled)
        logger.debug("::: Inside JetonWalletAuthStartedCron method :::");
        ...
        success.append("<u>Transaction Report <b>jeton wallet</b>" + "</u><br>");
        success.append("<br>");
        ...
        try
        {
            ...
            String wCond = "FROM transaction_common as t WHERE t.STATUS in ('authstarted','capturestarted') AND fromtype='jeton'  AND (TIMESTAMPDIFF(HOUR, FROM_UNIXTIME(dtstamp), CURRENT_TIMESTAMP)) > 1 ";
            ...
            if(isLogEnabled)
            transactionLogger.debug("Select Query jeton---" + ps+ "---authstartedCounter---"+authstartedCounter);
            ...
            while (resultSet.next())
            {
                ...
                auditTrailVO.setActionExecutorName("JetonWalletAuthStartedCron");
                ...
                JetonPaymentGateway jetonPaymentGateway = new JetonPaymentGateway(accountid);

                responseVO = (CommResponseVO)jetonPaymentGateway.processInquiry(trackingid, requestVO);
        ...
        catch (SQLException e)
        {
            ...
            PZExceptionHandler.raiseAndHandleDBViolationException(SkrillAuthStartedCron.class.getName(), "jetonWalletAuthStartedCron()", null, "common", "DB Exception", PZDBExceptionEnum.SQL_EXCEPTION, null, e.getMessage(), e.getCause(), toid, "jetonWalletAuthStartedCron");
            notProcessedCounter++;
        ...
        catch (PZDBViolationException pzd)
        {
            ...
            PZExceptionHandler.handleDBCVEException(pzd, toid, "jetonWalletAuthStartedCron");
            notProcessedCounter++;
            nTransaction.append(setTableData(trackingid,accountid,toid,sDescription,currency,amount,status));
        }
        catch (SystemError systemError)
        {
            ...
            PZExceptionHandler.raiseAndHandleDBViolationException(SkrillAuthStartedCron.class.getName(), "jetonWalletAuthStartedCron()", null, "common", "DB Exception", PZDBExceptionEnum.DB_CONNECTION_ISSUE, null, systemError.getMessage(), systemError.getCause(), toid, "jetonWalletAuthStartedCron");
            notProcessedCounter++;
            nTransaction.append(setTableData(trackingid, accountid, toid, sDescription, currency, amount, status));
