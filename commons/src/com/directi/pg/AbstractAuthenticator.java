...
public abstract class AbstractAuthenticator implements org.owasp.esapi.Authenticator {
    ...
    public void clearCurrent() {
         //logger.info(Logger.SECURITY_AUDIT, "************Clearing threadlocals. Thread" + Thread.currentThread().getName() );
    ...
    private User loginWithUsernameAndPassword(HttpServletRequest request) throws AuthenticationException {
        ...
        //String username = (String)request.getAttribute(ESAPI.securityConfiguration().getUsernameParameterName());
        String username = "";

        if(null!=request.getParameter(ESAPI.securityConfiguration().getUsernameParameterName()))
            username = request.getParameter(ESAPI.securityConfiguration().getUsernameParameterName());
        else if(null != (String)request.getAttribute(ESAPI.securityConfiguration().getUsernameParameterName()))
            username = (String)request.getAttribute(ESAPI.securityConfiguration().getUsernameParameterName());

        String password = (String)request.getAttribute(ESAPI.securityConfiguration().getPasswordParameterName());
    ...
    public void logout() {
        User user = getCurrentUser();
        logger.info(Logger.SECURITY_SUCCESS, "Inside AbstractAuthenticator: Current user " + user.getAccountName());
        if (user != null && !user.isAnonymous()) {
            user.logout();
    ...
    public void setCurrentUser(User user) {
        ...
        logger.info(Logger.SECURITY_SUCCESS, "Inside AbstractAuthenticator: Set Current user " + user.getAccountName());
