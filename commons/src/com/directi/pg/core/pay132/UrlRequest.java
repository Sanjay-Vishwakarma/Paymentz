...
public class UrlRequest {
    ...
    private static TransactionLogger transactionLogger = new TransactionLogger(UrlRequest.class.getName());
	...
	public String sendAndReturnString() throws PZTechnicalViolationException
    {
		ByteBuffer result = send();
		return result == null ? null : result.toString();
	}

	public ByteBuffer send() throws PZTechnicalViolationException
    {
		...
		for (int attempt = 0; status_code == -1 && attempt < getRetry() || response_ == 5;) {
			...
			try {
				...
				transactionLogger.error((new StringBuilder(String.valueOf(getUrl()))).append(": SSLHandshakeException encountered!").toString(), e);
                PZExceptionHandler.raiseTechnicalViolationException(UrlRequest.class.getName(),"send()",null,"common","Technical Exception", PZTechnicalExceptionEnum.SSL_HANDSHAKE_ISSUE,null,e.getMessage(),e.getCause());
				...
				transactionLogger.error((new StringBuilder(String.valueOf(getUrl()))).append(": SocketTimeoutException encountered! ").toString(), e);
                PZExceptionHandler.raiseTechnicalViolationException(UrlRequest.class.getName(),"send()",null,"common","Technical Exception", PZTechnicalExceptionEnum.GATEWAY_CONNECTION_TIMEOUT,null,e.getMessage(),e.getCause());
			} catch (IOException e) {
				response_ = 1;
				...
				transactionLogger.error((new StringBuilder(String.valueOf(getUrl()))).append(": IOException encountered!").toString(), e);
                PZExceptionHandler.raiseTechnicalViolationException(UrlRequest.class.getName(),"send()",null,"common","Technical Exception", PZTechnicalExceptionEnum.IOEXCEPTION,null,e.getMessage(),e.getCause());
			} catch (Exception e) {
				response_ = 1;
				...
				transactionLogger.error((new StringBuilder(String.valueOf(getUrl()))).append(": Exception encountered!").toString(), e);
                PZExceptionHandler.raiseTechnicalViolationException(UrlRequest.class.getName(),"send()",null,"common","Technical Exception", PZTechnicalExceptionEnum.NULL_POINTER_EXCEPTION,null,e.getMessage(),e.getCause());
			}
		}
	...
	public static void setNoTrustManager() {
		setTrustManager(new com.directi.pg.core.paylineVoucher.TrustAllCertificateSocketFactory());
	}
