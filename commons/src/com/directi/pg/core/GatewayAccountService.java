...
public class GatewayAccountService
{
    ...
    public String getGateway(String merchantId)
    {
        String gateway="";
        Connection conn = null;
        PreparedStatement preparedStatement = null;
        ...
        {
            conn = Database.getConnection();
            StringBuffer qry = new StringBuffer("select gateway from gateway_type where pgtypeid in (select pgtypeid from gateway_accounts where merchantid=?)");
            preparedStatement = conn.prepareStatement(qry.toString());
    ...
    public String getGatewayCountry(String accountId)
    {
        String country="";
        Connection conn = null;
        PreparedStatement preparedStatement = null;
        ...
        {
            conn = Database.getConnection();
            StringBuffer qry = new StringBuffer("select country from gateway_type where pgtypeid in (select pgtypeid from gateway_accounts where accountId=?)");
            preparedStatement = conn.prepareStatement(qry.toString());
            preparedStatement.setString(1, accountId);
            rs = preparedStatement.executeQuery();
            if (rs.next())
    ...
    public static TreeMap<String, String> getPaymodeIdGateway(String gatewayid)
    {
        Connection con = null;
        PreparedStatement psmt = null;
        Functions functions = new Functions();
        ...
        StringBuffer query = new StringBuffer("SELECT DISTINCT mam.paymodeid,p.paymentType FROM member_account_mapping AS mam, gateway_accounts AS ga, gateway_type AS gt,payment_type AS p WHERE mam.accountid=ga.accountid AND ga.pgtypeid=gt.pgtypeid AND p.paymodeid=mam.paymodeid");
        try
        {
            con = Database.getConnection();
            if (functions.isValueNull(gatewayid))
            ...
            query.append(" ORDER BY mam.paymodeid ASC");
            psmt = con.prepareStatement(query.toString());
            ResultSet rs = psmt.executeQuery();
            log.debug("Query::::::::::::::::"+psmt.toString());
            while (rs.next())
            {
                hash.put(rs.getString("paymodeid"),rs.getString("paymentType"));
            }
    ...
    public static TreeMap<String, String> getCardtypeIdGateway(String gatewayid)
    {
        Connection con = null;
        PreparedStatement psmt = null;
        Functions functions = new Functions();
        ...
        StringBuffer query = new StringBuffer("SELECT DISTINCT mam.cardtypeid,p.cardType FROM member_account_mapping AS mam, gateway_accounts AS ga, gateway_type AS gt,card_type AS p WHERE mam.accountid=ga.accountid AND ga.pgtypeid=gt.pgtypeid AND p.cardtypeid=mam.cardtypeid");
        try
        {
            con = Database.getConnection();
            if (functions.isValueNull(gatewayid))
            ...
            query.append(" ORDER BY mam.cardtypeid ASC");
            psmt = con.prepareStatement(query.toString());
            ResultSet rs = psmt.executeQuery();
            log.debug("Query::::::::::::::::"+psmt.toString());
            while (rs.next())
            {
                hash.put(rs.getString("cardtypeid"),rs.getString("cardType"));
            }
    ...
    public TreeMap<String, String> getParent_BankWireIdList(String accountid, String gatewayid)
    {
        String[] gatewayId = gatewayid.split("-");
        ...
        try {
            conn = Database.getConnection();
            StringBuffer query= new StringBuffer("SELECT * FROM bank_wiremanager WHERE parent_bankwireid>0 ");

            if (functions.isValueNull(gatewayId[2]))
            {
               query.append("AND pgtypeId=?"+Integer.parseInt(gatewayId[2]));
            }
            if (functions.isValueNull(accountid))
            {
               query.append("AND accountId =?" + Integer.parseInt(accountid));
            }
            pstmt= conn.prepareStatement(query.toString());

            ResultSet rs = pstmt.executeQuery();
            log.error("query======>" + pstmt);
            while (rs.next()) {

                    treeMap.put(String.valueOf(rs.getInt("parent_bankwireid")), String.valueOf(rs.getInt("parent_bankwireid")) + "-" + rs.getString("accountId") + "-" + rs.getString("mid"));
            }

        }   catch (SQLException se) {
            log.error("SQLException ::::::: Leaving AutoCompleteUtils ", se);
        } catch (SystemError se) {
    ...
    public TreeMap<String, String> getBankWireIdList()
    {
        TreeMap bankwirelist= new TreeMap();
        Connection connection= null;

       try
       {
           connection= Database.getConnection();
           String query= "select bankwiremanagerId,accountId,pgtypeId from bank_wiremanager order by bankwiremanagerId asc";
           PreparedStatement preparedStatement= connection.prepareStatement(query);
           log.error("Query for list of BankwireId::: "+query);
           ResultSet rs= preparedStatement.executeQuery();
           while (rs.next())
           {
               bankwirelist.put(String.valueOf(rs.getInt("bankwiremanagerId")),String.valueOf(rs.getInt("bankwiremanagerId"))+"-"+String.valueOf(rs.getInt("accountId"))+"-"+String.valueOf(rs.getInt("pgtypeId")));
           }
    ...
    public static String getGatewayLimitresetTime(String accountid)
    {
        String gatewayLimitresetTime="";
        Connection conn = null;
        PreparedStatement preparedStatement = null;
        ResultSet rs = null;
        try
        {
            conn = Database.getConnection();
            StringBuffer qry = new StringBuffer("select limitresettime from gateway_type where pgtypeid in (select pgtypeid from gateway_accounts where accountid=?)");
            preparedStatement = conn.prepareStatement(qry.toString());
            preparedStatement.setString(1, accountid);
            rs = preparedStatement.executeQuery();
            if (rs.next())
            {
                gatewayLimitresetTime=rs.getString("limitresettime");
                log.error("gatewayLimitresetTime--->"+gatewayLimitresetTime);
            }
        }
        catch (SystemError se)
        {
