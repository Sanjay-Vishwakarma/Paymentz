 ...
 */
public class PerfectMoneyLogger extends Logger
{
    private static Logger logger= new Logger("perfectmoney.test");

    private static volatile RollingFileAppender rollingFileAppender =setTransactionLogFileAppender() ;

    public PerfectMoneyLogger(String classname)
    {
        super(classname);
        try
        {
            cat.addAppender(rollingFileAppender);
            cat.setAdditivity(LogManager.getLogger("perfectmoneylog").getAdditivity());
        }
        catch (Exception e )
        {
    ...
    {
        RollingFileAppender transactionLogAppender= (RollingFileAppender) LogManager.getLogger("perfectmoneylog").getAppender("perfectmoneyProcess");
