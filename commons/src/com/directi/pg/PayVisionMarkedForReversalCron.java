 ...
 */
public class PayVisionMarkedForReversalCron
{
    private static Logger logger = new Logger(PayVisionMarkedForReversalCron.class.getName());
    private static TransactionLogger transactionLogger = new TransactionLogger(PayVisionMarkedForReversalCron.class.getName());
    boolean isLogEnabled= Boolean.parseBoolean(ApplicationProperties.getProperty("IS_LOG_ENABLED"));


    public String payVisionMarkedForReversalCron(Hashtable hashtable)
    {
        if (isLogEnabled)
            logger.debug("::: Inside payVisionMarkedForReversalCron method :::");
        ...
        try
        {
            ...
            String wCond = "FROM transaction_common as t WHERE t.STATUS in ('markedforreversal') AND fromtype='PayVision' AND (TIMESTAMPDIFF(HOUR, FROM_UNIXTIME(dtstamp), CURRENT_TIMESTAMP))>1";
            ...
            rs.next();
            refundCounter = rs.getInt(1);

            if (isLogEnabled)
                transactionLogger.debug("Select Query payvsion---" + ps+"----markedforreversalCounter---"+refundCounter);
            ...
            while (resultSet.next())
            {
                ...
                transactionLogger.debug("timestamp------" + timestamp);
                AuditTrailVO auditTrailVO = new AuditTrailVO();
                auditTrailVO.setActionExecutorId(toid);
                auditTrailVO.setActionExecutorName("payVisionMarkedForReversalCron");
                ...
                if (isLogEnabled)
                    transactionLogger.debug("Select Query payvision-------" + trackingid);

                PayVisionPaymentGateway payVisionPaymentGateway = new PayVisionPaymentGateway(accountid);

                responseVO = (CommResponseVO) payVisionPaymentGateway.processQuery(trackingid+"_R", requestVO);
        ...
        catch (SQLException e)
        {
            ...
            PZExceptionHandler.raiseAndHandleDBViolationException(PayVisionMarkedForReversalCron.class.getName(), "PayVisionMarkedForReversalCron()", null, "common", "DB Exception", PZDBExceptionEnum.SQL_EXCEPTION, null, e.getMessage(), e.getCause(), toid, "PayVisionMarkedForReversalCron");
            notProcessedCounter++;
        ...
        catch (PZDBViolationException pzd)
        {
            ...
            PZExceptionHandler.handleDBCVEException(pzd, toid, "PayVisionMarkedForReversalCron");
            notProcessedCounter++;
            nTransaction.append(setTableData(trackingid,accountid,toid,sDescription,currency,amount,status));
        }
        catch (SystemError systemError)
        {
            ...
            PZExceptionHandler.raiseAndHandleDBViolationException(PayVisionMarkedForReversalCron.class.getName(),"PayVisionMarkedForReversalCron()",null,"common","DB Exception", PZDBExceptionEnum.DB_CONNECTION_ISSUE,null,systemError.getMessage(),systemError.getCause(),toid,"PayVisionMarkedForReversalCron");
            notProcessedCounter++;
            nTransaction.append(setTableData(trackingid,accountid,toid,sDescription,currency,amount,status));
        ...
        success.append("<b>Total Transactions in MarkedForReversal : </b>"+refundCounter+"<br>");
        success.append("<br>");
        ...
        {
            sHeader.append("<br>");
            sHeader.append("<b><u>Transaction status changed from MarkedForReversal Started to Orders not found</b></u><br>");
            sHeader.append("<br>");
            sHeader.append(getTableHeader());
            sHeader.append("<br>");
            success.append("<br>");
        ...
        asynchronousMailService.sendEmail(MailEventEnum.ADMIN_MARKEDFORREVERSAL_CRON_REPORT, "", "", success.toString(), null);
