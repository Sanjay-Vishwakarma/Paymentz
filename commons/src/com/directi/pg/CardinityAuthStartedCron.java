 ...
 */
public class CardinityAuthStartedCron
{
    private static Logger logger = new Logger(CardinityAuthStartedCron.class.getName());
    private static TransactionLogger transactionLogger = new TransactionLogger(CardinityAuthStartedCron.class.getName());
    ...
    {
        CardinityAuthStartedCron cron = new CardinityAuthStartedCron();
        String sResponse = cron.cardinityAuthStartedCron(new Hashtable());
    }

    public String cardinityAuthStartedCron(Hashtable hashtable)
    {
        if (isLogEnabled)
            logger.debug("::: Inside cardinityAuthStartedCron method :::");
        ...
        success.append("<u>Transaction Report <b>cardinity</b>" + "</u><br>");
        success.append("<br>");
        ...
        try
        {
            ...
            String wCond = "FROM transaction_common as t WHERE t.STATUS in ('authstarted') AND fromtype='cardinity' AND (TIMESTAMPDIFF(HOUR , FROM_UNIXTIME(dtstamp), CURRENT_TIMESTAMP)) >1";
            ...
            if (isLogEnabled)
                transactionLogger.debug("Select Query cardinity---" + ps+"----authstarted---"+authstartedCounter);
            ...
            while (resultSet.next())
            {
                ...
                auditTrailVO.setActionExecutorName("cardinityAuthStartedCron");
                ...
                commTransactionDetailsVO.setPrevTransactionStatus(status);

                requestVO.setTransDetailsVO(commTransactionDetailsVO);
                //requestVO.getCommMerchantVO().setMerchantId("108");
                if (isLogEnabled)
                    transactionLogger.debug("Cardinity tracking -------" + trackingid +" paymentid---- "+paymentId);

                if(functions.isValueNull(paymentId))
                {
                    CardinityPaymentGateway cardinityPaymentGateway = new CardinityPaymentGateway(accountid);

                    responseVO = (CommResponseVO) cardinityPaymentGateway.processInquiry(requestVO);

                    transactionLogger.debug("status-----"+responseVO.getStatus());
                    ...
                    if (responseVO != null)
                    {
                        if (isLogEnabled)
                            if (responseVO.getStatus().equalsIgnoreCase("success"))
                            {
                                ...
                                if (responseVO.getTransactionType().equalsIgnoreCase(PZProcessType.AUTH.toString()))
                                {
                                    //Auth Success
                                ...
                                if (responseVO.getTransactionType().equalsIgnoreCase(PZProcessType.SALE.toString()))
                                {
                                    //Capture Success
        ...
        catch (SQLException e)
        {
            ...
            PZExceptionHandler.raiseAndHandleDBViolationException(CardinityAuthStartedCron.class.getName(), "cardinityAuthStartedCron()", null, "common", "DB Exception", PZDBExceptionEnum.SQL_EXCEPTION, null, e.getMessage(), e.getCause(), toid, "cardinityAuthStartedCron");
            notProcessedCounter++;
            nTransaction.append(setTableData(trackingid,accountid,toid,sDescription,currency,amount,status));
        }
        ...
        catch (PZDBViolationException pzd)
        {
            ...
            PZExceptionHandler.handleDBCVEException(pzd, toid, "cardinityAuthStartedCron");
            notProcessedCounter++;
            nTransaction.append(setTableData(trackingid,accountid,toid,sDescription,currency,amount,status));
        }
        catch (SystemError systemError)
        {
            ...
            PZExceptionHandler.raiseAndHandleDBViolationException(CardinityAuthStartedCron.class.getName(),"cardinityAuthStartedCron()",null,"common","DB Exception", PZDBExceptionEnum.DB_CONNECTION_ISSUE,null,systemError.getMessage(),systemError.getCause(),toid,"cardinityAuthStartedCron");
            notProcessedCounter++;
            nTransaction.append(setTableData(trackingid,accountid,toid,sDescription,currency,amount,status));
