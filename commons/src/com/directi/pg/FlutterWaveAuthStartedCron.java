...
/**
 * Created by Jitendra on 11/28/17.
 */
public class FlutterWaveAuthStartedCron
{
    private static FlutterWaveLogger transactionLogger= new FlutterWaveLogger(FlutterWaveAuthStartedCron.class.getName());

    final static ResourceBundle RB = LoadProperties.getProperty("com.directi.pg.FlutterWave");

    public String flutterWaveAuthStartedCron(Hashtable hashtable)
    {
        transactionLogger.error("::: Inside flutterWaveAuthStartedCron method :::");
        ...
        success.append("<u>Transaction Report <b>FlutterWave</b>" + "</u><br>");
        success.append("<br>");
        ...
        try
        {
            ...
            String wCond = "FROM transaction_common as t WHERE t.STATUS in ('authstarted','capturestarted') AND fromtype='Flutter' AND FROM_UNIXTIME(dtstamp)<DATE_ADD(CURRENT_TIMESTAMP,INTERVAL -20 MINUTE) AND FROM_UNIXTIME(dtstamp)>DATE_ADD(CURRENT_TIMESTAMP,INTERVAL -80 MINUTE);";
            ...
            transactionLogger.error("Select Query Flutter---" + ps+ "--authstartedCounter---"+authstartedCounter);
            while (resultSet.next())
            {
                ...
                auditTrailVO.setActionExecutorName("FlutterWaveAuthStartedCron");
                ...
                transactionLogger.error("Select Query Flutter-------" +trackingid);
                FlutterWavePaymentGateway flutterWavePaymentGateway=new FlutterWavePaymentGateway(accountid);
                responseVO= (CommResponseVO) flutterWavePaymentGateway.processInquiry(requestVO);

                if(responseVO!=null)
                {
                    if(responseVO.getStatus().equalsIgnoreCase("success") && responseVO.getAuthCode().equalsIgnoreCase("00"))
                    {
                            transactionLogger.error("------ inside success-------"+responseVO.getAuthCode());
                    ...
                    else if(responseVO.getStatus().equalsIgnoreCase("fail")) // For Transaction no found.
                    {
                            transactionLogger.debug("-----inside else if && remark error-----");
                        notificationUrl = resultSet.getString("notificationUrl");
