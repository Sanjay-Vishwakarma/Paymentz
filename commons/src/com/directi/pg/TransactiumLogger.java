 ...
 */
public class TransactiumLogger extends Logger
{
    private static Logger logger= new Logger("transactium.test");

    private static volatile RollingFileAppender rollingFileAppender =setTransactionLogFileAppender() ;

    public TransactiumLogger(String classname)
    {
        super(classname);
        try
        {
            cat.addAppender(rollingFileAppender);
            cat.setAdditivity(LogManager.getLogger("transactiumlog").getAdditivity());
        }
        catch (Exception e )
        {
    ...
    {
        RollingFileAppender transactionLogAppender= (RollingFileAppender) LogManager.getLogger("transactiumlog").getAppender("transactiumProcess");
