 ...
 */
public class WirecardNAuthStartedCron
{

    private static Logger logger = new Logger(WirecardNAuthStartedCron.class.getName());
    private static TransactionLogger transactionLogger = new TransactionLogger(WirecardNAuthStartedCron.class.getName());
    ...
    {
        WirecardNAuthStartedCron wirecardNAuthStartedCron = new WirecardNAuthStartedCron();
        String sResponse = wirecardNAuthStartedCron.wirecardNAuthStartedCron(new Hashtable());
    }

    public String wirecardNAuthStartedCron(Hashtable hashtable)
    {
        if(isLogEnabled)
        logger.debug("::: Inside WirecardNAuthStartedCron method :::");
        ...
        success.append("<u>Transaction Report <b>Wirecard1</b>" + "</u><br>");
        success.append("<br>");
        ...
        try
        {
            ...
            String wCond = "FROM transaction_common as t WHERE t.STATUS in ('authstarted','capturestarted') AND fromtype='Wirecard1' AND (TIMESTAMPDIFF(HOUR, FROM_UNIXTIME(dtstamp), CURRENT_TIMESTAMP)) > 1";
            ...
            if(isLogEnabled)
            transactionLogger.debug("Select Query Wirecard1---" + ps+"----authstartedCounter---"+authstartedCounter);
            ...
            while (resultSet.next())
            {
                ...
                auditTrailVO.setActionExecutorName("wirecardNAuthStartedCron");
                ...
                if(isLogEnabled)
                transactionLogger.debug("Select Query Wirecard1-------"+trackingid);

                WireCardNPaymentGateway wireCardNPaymentGateway = new WireCardNPaymentGateway(accountid);

                responseVO = (CommResponseVO)wireCardNPaymentGateway.processQuery(trackingid, requestVO);
                ...
                if(responseVO!=null)
                {
                    if(responseVO.getStatus().equalsIgnoreCase("success"))
                    {
                        ...
                        if (responseVO.getTransactionType().equalsIgnoreCase("purchase"))
                        {
                            //Capture Success
                        ...
                        if (responseVO.getTransactionType().equalsIgnoreCase("capture-authorization"))
                        {
                            //Captured
                            transStatus = "capturesuccess";
                            paymentid = responseVO.getTransactionId();
                        ...
                        if (responseVO.getTransactionType().contains("void-authorization"))
                        {
                            //Cancelled
                        ...
                        if (responseVO.getTransactionType().contains("original-credit"))
                        {
                            //Payout/Refund/OCT
                        ...
                        if (responseVO.getTransactionType().contains("refund-purchase"))
                        {
                        ...
                        if (responseVO.getTransactionType().equalsIgnoreCase("check-enrollment"))
                        {
                            //Auth Success
                            transStatus = "pending3DConfirmation";
                            paymentid = responseVO.getTransactionId();
                            insertMainTableEntry(transStatus,responseVO.getRemark(),paymentid,trackingid);
                            //Detail Table Entry
                            entry.actionEntryForCommon(trackingid, amount, ActionEntry.ACTION_AUTHORISTION_STARTED, ActionEntry.STATUS_AUTHORISTION_STARTED, responseVO, auditTrailVO, null);
                            authsucessCounter++;

                            responseVO = (CommResponseVO)wireCardNPaymentGateway.processQuery(trackingid+"_1", requestVO);
        ...
        catch (SQLException e)
        {
            ...
            PZExceptionHandler.raiseAndHandleDBViolationException(WirecardNAuthStartedCron.class.getName(), "wirecardNAuthStartedCron()", null, "common", "DB Exception", PZDBExceptionEnum.SQL_EXCEPTION, null, e.getMessage(), e.getCause(), toid, "wirecardNAuthStartedCron");
            notProcessedCounter++;
        ...
        catch (PZDBViolationException pzd)
        {
            ...
            PZExceptionHandler.handleDBCVEException(pzd, toid, "wirecardNAuthStartedCron");
            notProcessedCounter++;
            nTransaction.append(setTableData(trackingid,accountid,toid,sDescription,currency,amount,status));
        }
        catch (SystemError systemError)
        {
            ...
            PZExceptionHandler.raiseAndHandleDBViolationException(WirecardNAuthStartedCron.class.getName(),"wirecardNAuthStartedCron()",null,"common","DB Exception", PZDBExceptionEnum.DB_CONNECTION_ISSUE,null,systemError.getMessage(),systemError.getCause(),toid,"wirecardNAuthStartedCron");
            notProcessedCounter++;
            nTransaction.append(setTableData(trackingid,accountid,toid,sDescription,currency,amount,status));
        ...
        int tAdd = authsucessCounter+authFailedCounter+captureSuccesCounter+refundCounter+payoutCounter+failedCounter+notProcessedCounter;
    ...
    {
        transactionLogger.debug("------ inside getTableHeader-------");
        StringBuffer sHeader = new StringBuffer();
