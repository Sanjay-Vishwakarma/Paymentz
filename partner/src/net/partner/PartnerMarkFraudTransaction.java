...
public class PartnerMarkFraudTransaction extends HttpServlet
{
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
    {
        ...
        try
        {
                ...
                if (functions.isValueNull(req.getParameter("toid")))
                {
                   /* if(functions.isValueNull(pid)&& partnerFunctions.isPartnerMemberMapped(req.getParameter("toid"), pid)){
                    ...
                    else
                    {
                        ...
                        System.out.println("toid +++"+req.getParameter("toid"));
                        error="Invalid Partner Member configuration.";
                        req.setAttribute("error",error);
                        rd.forward(req, res);
                        return;
                    }*/
                    if (functions.isValueNull(req.getParameter("pid")))
                    {
                        if (partnerFunctions.isPartnerMemberMapped(req.getParameter("toid"), req.getParameter("pid")))
                        {
                            toidList = req.getParameter("toid");
                        }
                        else
                        {
                            error = "Invalid Partner Member configuration." + EOL;
                        }
                    }
                    else if (!functions.isValueNull(req.getParameter("pid")))
                    {
                        if (partnerFunctions.isPartnerSuperpartnerMembersMapped(req.getParameter("toid"), (String) session.getAttribute("merchantid")))
                        {
                            toidList = req.getParameter("toid");
                        }
                        else
            ...
            if(functions.isValueNull(error)){
                req.setAttribute("errormes",error);
                rd.forward(req, res);
                return;
            }
           ...
           /* partnername   = partner1.getPartnername(partnerid);*/

                if (functions.isValueNull(req.getParameter("pid")))
                {
                    partnername = partnerFunctions.getPartnerNameFromPartnerid(pid);
                    log.debug("partnerName if+++"+ partnername);
                }
                else
                {
//                    System.out.println("partnername session++++"+session.getAttribute("partnerid"));
                    partnername = partnerFunctions.getPartnerNameFromPartnerid(String.valueOf(session.getAttribute("partnerid")));
                    log.debug("partnerName else +++"+ partnername);
