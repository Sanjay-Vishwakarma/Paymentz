...
//import com.manager.dao.ApplicationManagerDAO;
...
public class ListofAppMember  extends HttpServlet
{
    ...
    public void doPost(HttpServletRequest request,HttpServletResponse response) throws IOException,ServletException
    {
        ...
        try{
            ...
            String pid=null;
            pid = request.getParameter("partnerid");
            if(null!=request.getParameter("apptoid") && !"".equals(request.getParameter("apptoid")))
            {
                ...
                if(functions.isValueNull((String) request.getParameter("apptoid")))
                    memberId= request.getParameter("apptoid");
                else if(functions.isValueNull((String) request.getAttribute("apptoid")))
            ...
            else
            {
                ...
                if(functions.isValueNull(pid)){
                    paginationVO.setInputs("memberId="+pid);
                    paginationVO.setPageNo(Functions.convertStringtoInt(request.getParameter("SPageno"), 1));
                    paginationVO.setPage(ListofAppMember.class.getName());
                    paginationVO.setCurrentBlock(Functions.convertStringtoInt(request.getParameter("currentblock"), 1));
                    paginationVO.setRecordsPerPage(Functions.convertStringtoInt(request.getParameter("SRecords"),15));
                    applicationManagerVOList=applicationManager.getPartnersMerchantApplicationManagerVO1(pid, paginationVO);
                }else{
                    ...
                    paginationVO.setCurrentBlock(Functions.convertStringtoInt(request.getParameter("currentblock"), 1));
                    paginationVO.setRecordsPerPage(Functions.convertStringtoInt(request.getParameter("SRecords"),15));
                    applicationManagerVOList=applicationManager.getSuperPartnersMerchantApplicationManagerVO1(memberId, paginationVO);
                }
            }
/*
            //inserting MemberID & ApplicationID details
            applicationManagerVOList=applicationManager.getPartnersMerchantApplicationManagerVO(memberId);*/
            //response
