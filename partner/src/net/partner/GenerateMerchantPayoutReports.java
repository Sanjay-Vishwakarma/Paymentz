...
public class GenerateMerchantPayoutReports extends HttpServlet
{
    ...
    public void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        if ("next".equalsIgnoreCase(action))
        {
            ...
            System.out.println("settlementCycleId--------->"+settlementCycleId);
            System.out.println("bankWireId--------->"+bankWireId);
            try
            {
                ...
                if (!currencyConversionReceived)
                {
                    ...
                    if (settlementCurrSet.size() > 0)
                    {
                        ...
                        session.setAttribute("settlementCycleId",settlementCycleId);
               ...
               /*calling the merchant payout report method*/
                ...
                if (stringList.size() > 0)
                {
                    ...
                    if ("fail".equalsIgnoreCase(stringList.get(stringList.size() - 1)))
                    {
                        bankDao.updateTheStatus(settlementCycleId, "BankWireGenerated");
                        request.setAttribute("stringList", stringList);
