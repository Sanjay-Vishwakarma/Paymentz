...
/**
 * Created by Suneeta on 23/10/2018.
 */
public class BlockBin extends HttpServlet
{
    private static Logger logger = new Logger(BlockBin.class.getName());
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
    {
        ...
        try
        {
            ...
            if(startBin.compareTo(endBin)>0){
                sErrorMessage.append("Start Bin is greater than End Bin.");
            }
            ...
            if (sErrorMessage.length() > 0)
            {
                ...
                listOfBin = blacklistDAO.getBlackListBinForPartner(startBin, endBin, accountid, memberid, records, pageno,actionExecutorId,actionExecutorName, req);
                if ("upload".equalsIgnoreCase(action))
                {
                    if (listOfBin.size() <= 0)
                    {
                        ...
                        {
                            blacklistDAO.addBin(startBin, endBin, accountid, memberid,"","",actionExecutorId,actionExecutorName,role_interface);
                            msg = "Bin Blocked successfully";
                        }
                    }
                    else
                    {
                        errormsg = "Same Record is Already available.";
                    }
