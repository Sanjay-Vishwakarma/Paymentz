...
//  System.out.println("In Virtual CC currency List map - "+currencyListMap);
  ...
  List<String> currencyList = (List<String>) currencyListMap.get(paymodeId);

//  System.out.println("In VirtualCreditCards currency List - "+currencyList);
...
//    System.out.println("terminalMap-----"+terminalMap);
...
%>
<input type="hidden" name="<%=cardName%>" id="<%=cardName%>" value="<%=terminalid%>,<%=addressValidation%>,<%=addressDisplay%>,<%=currency%>">
<%
}
else
{
//  System.out.println("List---"+currencyList.size());
...
//      System.out.println("s---"+c);
      //currency = c ;
//      System.out.println("CC-" + cardName + "-" + c);

      terminalVO = (TerminalVO) terminalMap.get("CC-" + cardName + "-" + c);
...
%>
<input type="hidden" name="<%=cardName%>" id="<%=cardName%>" value="<%=terminalid%>,<%=addressValidation%>,<%=addressDisplay%>,<%=c%>">
<%
        }
      }
    }
...
else
{
  if(functions.isValueNull(currency))
  {
    ...
    terminalid = terminalVO.getTerminalId();
%>
<input type="hidden" name="<%=cardName%>" id="<%=cardName%>_DC" value="<%=terminalid%>,<%=addressValidation%>,<%=addressDisplay%>,<%=currency%>">
<%
}
else
{
...
//      System.out.println("s---"+c);
//      System.out.println("CC-" + cardName + "-" + c);
      terminalVO = (TerminalVO) terminalMap.get("DC-" + cardName + "-" + c);
      if(terminalVO!=null)
      {
        addressValidation = terminalVO.getAddressValidation();
        addressDisplay = terminalVO.getAddressDetails();
        terminalid = terminalVO.getTerminalId();
%>
<input type="hidden" name="<%=cardName%>" id="<%=cardName%>_DC" value="<%=terminalid%>,<%=addressValidation%>,<%=addressDisplay%>,<%=c%>">
<%
            }
          }
        }
...
<div class="tab-pane" id="<%=value%>">
  <form id="cardForm<%=id%>" class="form-style" method="post" >
    ...
    <input type="hidden" name="limitRouting" id="limitRouting<%=id%>"  value="<%=limitRouting%>">
    <input type="hidden" name="merchantOrderDetails" id="merchantOrderDetails<%=id%>" value="<%=standardKitValidatorVO.getMerchantDetailsVO().getMerchantOrderDetailsDisplay()%>">

    <div class="tab" id="personalinfo<%=id%>" style="padding-top: 22px;">
      ...
      <div class="form-group has-float-label control-group-half">
        ...
        <input type="text" class="form-control input-control1" id="phonenophonecc<%=id%>" placeholder=" " onkeypress="return isNumberKey(event)" onkeyup="onPasteNumCheck(event)"
    ...
    <div class="tab" id="cardinfo<%=id%>" style="padding-top: 22px;"> <p class="form-header" style="display: none;"><%=varCardinfo%></p>
      ...
      <div class="form-group has-float-label control-group-seventy" id="CardAmount<%=id%>" >
        <input type="text" class="form-control input-control1" id="amount<%=id%>" placeholder=" " onkeypress="return isNumberKey(event)"
               onfocusout="validateAmount(event.target.value ,'CardAmount<%=id%>')"
               oninput="this.className = 'form-control input-control1'" name="amount" value=""/>
        <label for="amount<%=id%>" class="form-label">Amount</label>
      </div>
      <div class="form-group has-float-label control-group-twenty-five" id="Currency<%=id%>" >
        <%
          if(functions.isValueNull(currency)) {
        %>
        <input DISABLED type="text" class="form-control input-control1" id="currency<%=id%>" placeholder=" "
               oninput="this.className = 'form-control input-control1'; currencyDropDown()" value="<%=currency%>"/>
        <input type="hidden" name="currency" value="<%=currency%>" />
        <%
        }
        else {
        %>
        <select name="currency" class="form-control input-control1" id="currencydropdown<%=id%>" >
          <%
        ...
        %>
        <label for="currency<%=id%>" class="form-label">Currency</label>
      </div>
      <div class="form-group has-float-label control-group-seventy card" id="CardNumber<%=id%>">
        <input class="form-control input-control1" id="cardNo<%=id%>" placeholder=" " type="text" onkeypress="return isNumberKey(event)" onkeyup="onPasteNumCheck(event)"
               onfocusout = "isValidCardCheck('cardNo<%=id%>', 'CardNumber<%=id%>','onfocusout')" <%--onfocusout = "funcCheck('cardNo', 'CardNumber')"--%> autocomplete="off"
          ...
          <div class="form-group has-float-label control-group-half" id="CardState<%=id%>" >
            <div class="dropdown">
              <input type="text" class="form-control input-control1" id="state<%=id%>" oninput="this.className = 'form-control input-control1';stateinp();" placeholder=" "
                     onblur="validateState(event.target.value ,'CardState<%=id%>')" name="state" value="<%=state%>"/>
