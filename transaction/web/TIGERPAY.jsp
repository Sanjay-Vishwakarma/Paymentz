...
%>
<%--
<div id="TIGERPAYOption" class="option-class">
  <ul class="nav nav-tabs" id="TIGERPAYTab">
    <%
    ...
    %>
    <li class="tabs-li-wallet" onclick="TIGERPAYHideShow('<%=cardName%>','<%=rb.getString(cardName)%>')">
...
<div class="tab-pane" id="<%=tabId%>" style="height: 305px;" >
  <form id="<%=tabId%>Form" class="form-style" method="post">
    ...
    <div class="tab" id="personalinfo" style="padding-top: 22px;height:auto;overflow:unset">
      ...
      <div class="form-group has-float-label control-group-half" id="f_name">
        <input type="text" class="form-control input-control1" id="firstname_nbi" placeholder=" "
               oninput="this.className = 'form-control input-control1'" name="firstname" value="<%=firstName%>" autofocus maxlength="50" autocomplete="off" onblur="validateName('firstname_nbi','f_name')" />
        <label for="firstname_nbi" class="form-label"><%=varFirstName%></label>
      </div>
      <div class="form-group has-float-label control-group-half" id="l_name">
        <input type="text" class="form-control input-control1" id="lastname_nbi" placeholder=" "
               oninput="this.className = 'form-control input-control1'" name="lastname" value="<%=lastName%>" autofocus maxlength="50" autocomplete="off" onblur="validateName('lastname_nbi','l_name')" />
      ...
      <ul class="nav nav-tabs" id="TIGERPAYTab">
        <%
          for(String cardId : cardList)
          {
            ...
            cardName        = cardName.replaceAll(" ", "_");
        %>
        <li class="tabs-li-wallet"  onclick="TIGERPAYHideShow('<%=cardName%>','<%=rb.getString(cardName)%>',true)">
          <a href="#<%=cardName%>"  data-toggle="tab" title="<%=cardName%>" aria-expanded="false">
            <%if(cardName.equalsIgnoreCase("JPBANK")){%>
                <img class="images-style" src="/images/merchant/TigerGateWay.png" alt="<%=cardName%>">
            <%}else{%>
