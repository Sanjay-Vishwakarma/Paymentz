...
<%@ page import="com.directi.pg.core.valueObjects.TigerPayResponseVO" %>
...
//        trackingId=commonValidatorVO.getTrackingid();
   ...
   // JPBankTransferVO jpBankTransferVO       = (JPBankTransferVO) request.getAttribute("transRespDetails");
    TigerPayResponseVO tigerPayResponseVO   = (TigerPayResponseVO) request.getAttribute("transRespDetails");
    String bankName = "";
    String branchName = "";
    String branchCode = "";
    String accountType = "";
    ...
    bankName        =  tigerPayResponseVO.getBankName();
    branchName      = tigerPayResponseVO.getBranchName();
    accountType     = tigerPayResponseVO.getAccountType();
    accountNumber   = tigerPayResponseVO.getAccountNumber();
    accountName     = tigerPayResponseVO.getAccountName();
    branchCode      = tigerPayResponseVO.getBranchCode();

    if(functions.isValueNull(tigerPayResponseVO.getTransactionId())){
        referenceNumber          = tigerPayResponseVO.getTransactionId();
    }
                    ...
                    <div class="top-bar">
                        <div class="header-section">
                            <div style="padding: 0 0 5px 0;">
                                ...
                                振込元名義：<span style="color:red">6-7桁のお振込ID + 口座名義</span>（例）1234567カワセタロウ
                            </div>
                            <div>
                    ...
                    <div>
                        ...
                        <div class="data-section">
                            ...
                            <%
                                ...
                                if(functions.isValueNull(branchName)){
                            %>
                            <div class="data-row">
                                <div class="data-label"> Branch Name 支店名 </div>
                                <div class="data-value"> <%=ESAPI.encoder().encodeForHTML(branchName)%> </div>
                            </div>
                            <%
                                }
                                if(functions.isValueNull(branchCode)){
                            %>
                            <div class="data-row">
                                <div class="data-label"> Branch Code 支店コード </div>
                                <div class="data-value"> <%=ESAPI.encoder().encodeForHTML(branchCode)%> </div>
                            </div>
                            <%
                                }
                                if(functions.isValueNull(accountType)){
                            %>
                            <div class="data-row">
                                <div class="data-label"> Account Type  口座の種類 </div>
                                <div class="data-value"> <%=ESAPI.encoder().encodeForHTML(accountType)%> </div>
                            </div>
                            <%
                                }
                                if(functions.isValueNull(accountNumber)){
                            %>
                            <div class="data-row">
                                <div class="data-label"> Account Number 口座番号 </div>
                                <div class="data-value"> <%=ESAPI.encoder().encodeForHTML(accountNumber)%> </div>
                            </div>
                            <%
                                }
                                if(functions.isValueNull(accountName)){
                            %>
                            <div class="data-row">
                                <div class="data-label"> Account Holder 口座名義 </div>
                                <div class="data-value"> <%=ESAPI.encoder().encodeForHTML(accountName)%> </div>
                            </div>
                            <%
                                }
                            %>
                            <div class="data-row">
                                <div class="data-label" style="color:red"> Reference Number 参照番号 </div>
                                <div class="data-value" style="color:red"> <%=ESAPI.encoder().encodeForHTML(referenceNumber)%> </div>
                            </div>
                            <%
