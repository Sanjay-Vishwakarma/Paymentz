 ...
 */
public class AirPayBackEndServlet extends HttpServlet
{
    private static TransactionLogger transactionLogger=new TransactionLogger(AirPayBackEndServlet.class.getName());
    ...
    {
        transactionLogger.error("----Inside AirPayBackEndServlet----");
                    ...
                    if (transactionDetailsVO != null)
                    {
                        ...
                        if (PZTransactionStatus.AUTH_STARTED.toString().equalsIgnoreCase(dbStatus) || PZTransactionStatus.AUTHSTARTED_3D.toString().equalsIgnoreCase(dbStatus) || (PZTransactionStatus.AUTH_FAILED.toString().equalsIgnoreCase(dbStatus) &&"Success".equalsIgnoreCase(txnStatus)))
                        {
                            ...
                            if ("16".equalsIgnoreCase(status))
                            {
                                ...
                                dbBuffer.append("update transaction_common set captureamount='" + respAmount + "',currency='" + currency + "' ,paymentid='" + transactionId + "',status='capturesuccess' ,remark='" + ESAPI.encoder().encodeForSQL(me, message) + "' where trackingid = " + trackingId);
