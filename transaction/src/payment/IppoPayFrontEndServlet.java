 ...
 */
public class IppoPayFrontEndServlet extends HttpServlet
{

    //private static TransactionLogger transactionlogger = new TransactionLogger(IppoPayFrontEndServlet.class.getName());
    private static IppoPayTransactionLogger transactionlogger = new IppoPayTransactionLogger(IppoPayFrontEndServlet.class.getName());
    ...
    {
        transactionlogger.error("Entering IppoPayFrontEndServlet ......");
        ...
        {
            transactionlogger.error("----for loop IppoPayBackEndServlet-----" + key + "=" + req.getParameter((String) key) + "----------");
        ...
        try{
           ...
           /* String INTERNAL_ACQUIRER_TYPE=req.getParameter("INTERNAL_ACQUIRER_TYPE");//for PhonePe
            ...
            JSONObject transactionResponse = new JSONObject(req.getParameter("data"));
                 if(transactionResponse.has("status"))
                 {
                     tranStatus = transactionResponse.getString("status");
                     transactionlogger.error("tranStatus----->" + tranStatus);

                 }
            if(transactionResponse.has("transaction_no")){
                tranhashinfo = transactionResponse.getString("transaction_no");
                transactionlogger.error("tranhashinfo----->" + tranhashinfo);
            }
            ...
            if (transactionDetailsVO != null)
            {
                ...
                transactionlogger.error("accountid in side IppoPay frontEnd------------------------------------->"+accountId);
              ...
              //  displayName = GatewayAccountService.getGatewayAccount(accountId).getDisplayName();
                ...
                {
                    transactionlogger.error("accountid  IppoPay frontend-------------------------------------->"+accountId);
                    IppoPayPaymentGateway ippoPayPaymentGateway=new IppoPayPaymentGateway(accountId);
                  ...
                  //  commResponseVO= (CommResponseVO) ippoPayPaymentGateway.processInquiry(requestVO);
                    ...
                    StringBuffer dbBuffer = new StringBuffer();
                        ...
                        commResponseVO.setDescription(tranStatus);
                        commResponseVO.setRemark(tranStatus);
                        commResponseVO.setAmount(amount);
                        commResponseVO.setCurrency(currency);
                        commResponseVO.setTransactionId(tranhashinfo);
                        ...
                        if ("success".equalsIgnoreCase(tranStatus))
                        {
                            ...
                            dbBuffer.append("update transaction_common set captureamount='" + amount + "',status='capturesuccess',customerIp='" + requestIp + "',customerIpCountry='" + functions.getIPCountryShort(requestIp) + "',remark='" + tranStatus + "' where trackingid = " + trackingId);
                        ...
                        else if("pending".equalsIgnoreCase(tranStatus))
                        {
                            transactionlogger.error("inside transactionStatus success");
                            status = "pending";
                            dbStatus = "pending";
                        ...
                        else
                        {
                            ...
                            billingDesc="";
                            displayName="";
                            dbBuffer.append("update transaction_common set amount='" + amount + "',status='authfailed',customerIp='" + requestIp + "',customerIpCountry='" + functions.getIPCountryShort(requestIp) + "', remark = '" + tranStatus + "'where trackingid = " + trackingId);
                            con = Database.getConnection();
                            Database.executeUpdate(dbBuffer.toString(), con);
        ...
        catch(Exception e)
        {
            ...
            PZExceptionHandler.raiseAndHandleDBViolationException("vervepayfrontendservlet.java", "doService()", null, "Transaction", null, PZDBExceptionEnum.INCORRECT_QUERY, null, e.getMessage(), e.getCause(), toid, null);
        }
