...
/**
 * Created by Uday on 4/24/18.
 */
public class AWFrontEndServlet extends HttpServlet
{
    TransactionLogger transactionLogger= new TransactionLogger(AWFrontEndServlet.class.getName());
    ...
    public void doService(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        try
        {
            ...
            AlliedPaymentProcess paymentProcess = new AlliedPaymentProcess();
            CommRequestVO commRequestVO = new CommRequestVO();

            paymentProcess.setAWRequestVO(commRequestVO, trackingId);
            ...
            AlliedWalletPaymentGateway alliedWalletPaymentGateway = new AlliedWalletPaymentGateway(accountId);
            ...
            try
            {
                if (PZTransactionStatus.AUTH_STARTED.toString().equals(dbStatus))
                {
                    ...
                    if ("N".equals(isService))
                    {
                        ...
                        transRespDetails = (CommResponseVO) alliedWalletPaymentGateway.process3DAuthConfirmation(commRequestVO, PARes);
                        transType = "Auth";
                    }
                    else
                    {
                        entry.actionEntryFor3DCommon(trackingId, amount, ActionEntry.ACTION_3D_CONFIRMATION_STARTED, ActionEntry.STATUS_3D_CONFIRMATION, transRespDetails, commRequestVO, auditTrailVO, null);
                        transRespDetails = (CommResponseVO) alliedWalletPaymentGateway.process3DSaleConfirmation(commRequestVO, PARes);
                    }
            ...
            catch (SystemError se)
            {
                ...
                PZExceptionHandler.raiseAndHandleDBViolationException("AWFrontEndServlet.java", "doService()", null, "Transaction", null, PZDBExceptionEnum.DB_CONNECTION_ISSUE, null, se.getMessage(), se.getCause(), toId, null);
            }
            finally
            {
        ...
        catch (PZTechnicalViolationException e)
        {
            ...
            PZExceptionHandler.raiseAndHandleTechnicalViolationException("AWFrontEndServlet.java", "doService()", null, "Transaction", null, PZTechnicalExceptionEnum.IOEXCEPTION, null, e.getMessage(), e.getCause(), toId, null);
        }

        catch (PZDBViolationException e)
        {
            transactionLogger.error("error::::", e);
            PZExceptionHandler.raiseAndHandleDBViolationException("AWFrontEndServlet.java", "doService()", null, "Transaction", null, PZDBExceptionEnum.INCORRECT_QUERY, null, e.getMessage(), e.getCause(), toId, null);
        }
    }
}
