 ...
 */
public class PTMFrontEndServlet extends HttpServlet
{
    private static TransactionLogger transactionlogger = new TransactionLogger(PTMFrontEndServlet.class.getName());
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
    {
        ...
        {
            transactionlogger.error("Entering PTMFrontEndServlet ......", e);
        }
    }
    ...
    {
        transactionlogger.error("Entering PTMFrontEndServlet ......");
        ...
        try
        {
            ...
            if (transactionDetailsVO != null)
            {
                ...
                if (PZTransactionStatus.AUTH_STARTED.toString().equals(dbStatus))
                {
                    ...
                    commResponseVO = (CommResponseVO) payTMPaymentGateway.processQuery(trackingId,requestVO);
                    ...
                    if (functions.isValueNull(responseAmount))
                    {
                        ...
                        else if (!compDbAmount.equals(compRsAmount) && "success".equalsIgnoreCase(inStatus))
                        {
                            ...
                            blacklistVO.setActionExecutorName("PTMFrontEndServlet");
                    ...
                    {
                        notificationUrl = transactionDetailsVO.getNotificationUrl();
                        transactionlogger.error("inside Lyra failed --->");
                        status = "Failed";
                        message = remark;
                        responseStatus = remark;
                        commResponseVO.setStatus(status);
                    ...
                    else
                    {
                        ...
                        status = "Pending";
                        message = "Pending";
                        transactionlogger.error("inside PTMFrontEndServlet pending condition --->" + STATUS + "--" + message);
                    }
                ...
                else
                {
                    ...
                    {
                        transactionlogger.error("inside else success  condition paytm --->" + dbStatus + "--" +" trackingid  "+trackingId);
...
*/
                ...
                if(functions.isValueNull(websiteurl)&&!"https://".equalsIgnoreCase(websiteurl)){
               ...
               else  if (functions.isValueNull(genericTransDetailsVO.getRedirectUrl()))
                {
                    transactionlogger.error("responseStatus in ---" + dbStatus);
                    payTMUtils.doHttpPostConnection(commonValidatorVO, redirectUrl, billingDesc, status);
                    transactionUtility.doAutoRedirect(commonValidatorVO, res, status, billingDesc);
                }
        ...
        {
            transactionlogger.error("PZTechnicalViolationException paytm---", e);
        }
        catch (NoSuchAlgorithmException e)
        {
            transactionlogger.error("NoSuchAlgorithmException  paytm---",e);
        }
        catch (SystemError systemError)
        {
            transactionlogger.error("SystemError  paytm---",systemError);
        }
        catch (Exception e)
        {
            transactionlogger.error("Exception  paytm---", e);
        }
    }
}
