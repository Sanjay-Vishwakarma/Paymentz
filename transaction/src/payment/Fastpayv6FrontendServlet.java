 ...
 */
public class Fastpayv6FrontendServlet extends PzServlet
{
    private static TransactionLogger transactionLogger = new TransactionLogger(Fastpayv6FrontendServlet.class.getName());
    String ctoken = ESAPI.randomizer().getRandomString(32, DefaultEncoder.CHAR_ALPHANUMERICS);

    public Fastpayv6FrontendServlet()
    {
        super();
    }
    ...
    {
        transactionLogger.error("-----Entering into Fastpayv6FrontendServlet-----");
        ...
        if (xmlResponse != null)
        {
            ...
            try
            {
                Map<String, String> stringStringMap = ApcoFastPayV6Utils.readFastPayV6RedirectionXMLResponse(xmlResponse);
                if(functions.isValueNull(stringStringMap.get("ORef")))
                {
                ...
                transactionLogger.error("----- resultCode id ----- "+resultCode);
                transactionLogger.error("----- pspid id ----- "+pspid);
                transactionLogger.error("----- captureAmount id ----- "+captureAmount);
 ...
 /*
                  ...
                  //  transactionLogger.debug("num------"+ccnum);
                    ...
                    transactionLogger.error("FrontEnd ----> " + trackingId + "dbStatus --" + dbStatus);
                    ...
                    {
                        ApcoFastPayV6PaymentGateway apcoFastPayV6PaymentGateway = new ApcoFastPayV6PaymentGateway(accountId);
                        ...
                        transactionLogger.error("Result code  of Transaction=========="+trackingId+" Result "+resultCode);
                       ...
                       /* if(resultCode.equalsIgnoreCase("OK") || resultCode.equalsIgnoreCase("CAPTURED") || resultCode.equalsIgnoreCase("APPROVED") || resultCode.equalsIgnoreCase("PROCESSED"))
                        ...
                        commResponseVO  = (CommResponseVO) apcoFastPayV6PaymentGateway.processInquiry(commRequestVO);
                        ...
                        if (commResponseVO != null)
                        {
                            ...
                            transactionLogger.error("Staus of Transaction=========="+trackingId+" Result "+resStatus +"transaction Id"+transactionId );
                            ...
                            {    notificationUrl    = transactionDetailsVO.getNotificationUrl();
                                ...
                                billingDesc         = "";
                                displayName         = "";
                                dbBuffer.append("update transaction_common set paymentid='" + commResponseVO.getResponseHashInfo() + "',status='authfailed',remark = '" + message + "'");
                    ...
                    transactionLogger.error("log 1 " + trackingId);
                    addressDetailsVO.setFirstname(firstName);
                    transactionLogger.error("log 2 " + trackingId);
                    addressDetailsVO.setLastname(lastName);
                    transactionLogger.error("log 3 " + trackingId);
                    ...
                    TransactionDetailsVO transactionDetailsVO3 = transactionManager.getTransDetailFromCommon(trackingId);
                    if("authstarted".equalsIgnoreCase(transactionDetailsVO3.getStatus())){
                        respStatus="pending";
                    }else{
                        respStatus=transactionDetailsVO3.getStatus();
                    }

                  /*  AsynchronousMailService asynchronousMailService = new AsynchronousMailService();
                    asynchronousMailService.sendEmail(MailEventEnum.PARTNERS_MERCHANT_SALE_TRANSACTION, String.valueOf(trackingId), respStatus, message, billingDesc);
                    AsynchronousSmsService smsService = new AsynchronousSmsService();
                    smsService.sendSMS(MailEventEnum.PARTNERS_MERCHANT_SALE_TRANSACTION, String.valueOf(trackingId), respStatus, message, billingDesc);
                 */   if (functions.isValueNull(notificationUrl))
                    {
                        transactionLogger.error("inside sending notification---" + notificationUrl);
                       // TransactionDetailsVO transactionDetailsVO3 = transactionManager.getTransDetailFromCommon(trackingId);
                       //TransactionDetailsVO transactionDetailsVO1 = transactionUtility.getTransactionDetails(commonValidatorVO);
                        transactionDetailsVO3.setTransactionMode("3D");
                        AsyncNotificationService asyncNotificationService = AsyncNotificationService.getInstance();
                        asyncNotificationService.sendNotification(transactionDetailsVO3, trackingId, respStatus, message,"");
                    }
            ...
            {
                transactionLogger.error("SQL Exception in Fastpayv6FrontendServlet---", e);
            }
            catch (SystemError e)
            {
                transactionLogger.error("System Exception in Fastpayv6FrontendServlet---", e);
            }
            catch (NoSuchAlgorithmException e)
            {
                transactionLogger.error("NoSuchAlgorithm Exception in Fastpayv6FrontendServlet---", e);
            }
