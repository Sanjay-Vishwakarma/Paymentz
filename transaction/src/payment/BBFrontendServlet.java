 ...
 */
public class BBFrontendServlet extends HttpServlet
{
    private static TransactionLogger transactionlogger = new TransactionLogger(BBFrontendServlet.class.getName());
    ...
    public void doGet(HttpServletRequest req,HttpServletResponse res) throws ServletException, IOException
    {
        ...
        {
            transactionlogger.error("Entering BBFrontendServlet ......",e);
        }
    }
    ...
    {
        transactionlogger.error("....Entering BBFrontendServlet ......");
        ...
        try
        {
            transactionlogger.error("----- BBFrontend Response -----"+trackingId);
            Enumeration enumeration = req.getParameterNames();
            while (enumeration.hasMoreElements())
            {
                String key      = (String) enumeration.nextElement();
                String value   ="";
                if(key.equalsIgnoreCase("trackingId")){
...
//            Thread.sleep(5000);
            ...
            if (transactionDetailsVO != null)
            {
                ...
                auditTrailVO.setActionExecutorName("BBFrontend");
                auditTrailVO.setActionExecutorId(toid);

                BoomBillPaymentGateway boomBillPaymentGateway  = new BoomBillPaymentGateway(accountId);
                ...
                if (PZTransactionStatus.AUTH_STARTED.toString().equals(dbStatus) || PZTransactionStatus.AUTHSTARTED_3D.toString().equalsIgnoreCase(dbStatus))
                {
                    ...
                    comm3DResponseVO  = (Comm3DResponseVO) boomBillPaymentGateway.processQuery(trackingId, requestVO);
                    ...
                    String descriptor    = comm3DResponseVO.getDescriptor();
                    ...
                    /*if(functions.isValueNull(responseAmount))
                    {
                        ...
                        {
                            transactionlogger.error(trackingId+" BBFrontend inside else Amount incorrect--->" + responseAmount);
                            remark = "Failed-IRA";
                            STATUS = "failed";
                    ...
                    if("success".equalsIgnoreCase(inStatus))
                    {
                        transactionlogger.error("sucess  front end operation: "+inStatus);
                    ...
                    else if("failed".equalsIgnoreCase(inStatus))
                    {
                        transactionlogger.error("failed  front end operation: "+ inStatus);
                        notificationUrl = transactionDetailsVO.getNotificationUrl();
                        status          = "failed";
                        message         = remark;
                        ...
                        dbBuffer.append("',authorization_code='"+amount+"',customerIp='" + requestIp + "',customerIpCountry='" + functions.getIPCountryShort(requestIp) + "' ,remark='" + ESAPI.encoder().encodeForSQL(me, message)+ "' where trackingid = " + trackingId);
                        transactionlogger.error("Update Query---" + dbBuffer);
                        Database.executeUpdate(dbBuffer.toString(), con);
                ...
                else
                {
                    ...
                    {
                        transactionlogger.error("inside BBFrontendServlet transactionStatus " + transactionStatus);
                        status      = "pending";
...
//                smsService.sendSMS(MailEventEnum.PARTNERS_MERCHANT_SALE_TRANSACTION, String.valueOf(trackingId), status, message, billingDesc);
        ...
        catch (Exception e)
        {
            ...
            PZExceptionHandler.raiseAndHandleDBViolationException("BBFrontendServlet.java", "doService()", null, "Transaction", null, PZDBExceptionEnum.INCORRECT_QUERY, null, e.getMessage(), e.getCause(), toid, null);
        }
        finally
        {
