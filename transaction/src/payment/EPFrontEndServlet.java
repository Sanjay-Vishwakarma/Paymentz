 ...
 */
public class EPFrontEndServlet extends HttpServlet
{
    private static TransactionLogger transactionlogger = new TransactionLogger(EPFrontEndServlet.class.getName());
    ...
    public void doGet(HttpServletRequest req,HttpServletResponse res) throws ServletException, IOException
    {
        ...
        {
            transactionlogger.error("Entering EPFrontEndServlet ......",e);
        }
    }
    ...
    {
        transactionlogger.error("....Entering EPFrontEndServlet ......");
        ...
        try
        {
            ...
            transactionlogger.error(trackingId +" ----- EPFrontEndServlet Response -----" + responseMsg);

            String splitStatus      = "";
            String splitTrackingId  = "";

            transactionlogger.error("combined trackingId with status: " + trackingId);
            ...
            if (transactionDetailsVO != null)
            {
                ...
                if (functions.isValueNull(transactionDetailsVO.getBankCode())){
                    bankcode  = transactionDetailsVO.getBankCode();
                }
                if (functions.isValueNull(transactionDetailsVO.getBankDescription())){
                    bankDescription = transactionDetailsVO.getBankDescription();
                }
                ...
                auditTrailVO.setActionExecutorName("EPFrontEnd");
                auditTrailVO.setActionExecutorId(toid);

                EasyPaymentGateway easyPaymentGateway     = new EasyPaymentGateway(accountId);
                ...
                if (PZTransactionStatus.AUTH_STARTED.toString().equals(dbStatus))
                {
                    ...
                    comm3DResponseVO  = (Comm3DResponseVO) easyPaymentGateway.processQuery(splitTrackingId, requestVO);
                    ...
                    if (functions.isValueNull(comm3DResponseVO.getBankCode())){
                        bankcode = comm3DResponseVO.getBankCode();
                    }

                    if (functions.isValueNull(comm3DResponseVO.getBankDescription())){
                        bankDescription = comm3DResponseVO.getBankDescription();
                    }
                    ...
                    transactionlogger.error("inquiry trackingid--->"+splitTrackingId);
                    ...
                    if(functions.isValueNull(responseAmount))
                    {
                        ...
                        {
                            transactionlogger.error(trackingId + " EPFrontend inside else Amount incorrect--->" + responseAmount);
                            remark = "Failed-IRA";
                            STATUS = "Failed";
                            ...
                            blacklistVO.setRemark("IncorrectAmount Trackingid : "+splitTrackingId);
                    ...
                    if ("Success".equalsIgnoreCase(STATUS) && "payin".equalsIgnoreCase(operation))
                    {
                        ...
                        dbBuffer.append("',customerIp='" + requestIp + "',customerIpCountry='" + functions.getIPCountryShort(requestIp) + "' ,remark='" + message + "' where trackingid = " + splitTrackingId);
                        transactionlogger.error("Update Query---" + dbBuffer);

                        Database.executeUpdate(dbBuffer.toString(), con);
                        entry.actionEntryForCommon(splitTrackingId, amount, ActionEntry.ACTION_CAPTURE_SUCCESSFUL, ActionEntry.STATUS_CAPTURE_SUCCESSFUL, comm3DResponseVO, auditTrailVO, requestIp);//+ ",rrn='" + rrn
                    }
                    else if ("pending".equalsIgnoreCase(STATUS) && "auth".equalsIgnoreCase(operation))
                    {
                        transactionlogger.error("PENDING auth  front end operation: " + operation +" "+STATUS);
                        notificationUrl    = transactionDetailsVO.getNotificationUrl();
                        status              = "success";
                        billingDesc         = displayName;
                        message             = remark;
                        ...
                        dbBuffer.append("update transaction_common set paymentid='" + TXN_ID + "',templateamount='" + tmpl_amt + "',templatecurrency='" + tmpl_currency +  "',rrn='" +RRN+ "" +"',status='authsuccessful'" + " ,successtimestamp='" + functions.getTimestamp());
                        dbBuffer.append("',customerIp='" + requestIp + "',customerIpCountry='" + functions.getIPCountryShort(requestIp) + "' ,remark='" + message + "' where trackingid = " + splitTrackingId);
                        transactionlogger.error("Update Query---" + dbBuffer);

                        Database.executeUpdate(dbBuffer.toString(), con);
                        entry.actionEntryForCommon(splitTrackingId, amount, ActionEntry.ACTION_AUTHORISTION_SUCCESSFUL, ActionEntry.STATUS_AUTHORISTION_SUCCESSFUL, comm3DResponseVO, auditTrailVO, requestIp);//+ ",rrn='" + rrn

                    }
                    else if("Failed".equalsIgnoreCase(STATUS) || "declined".equalsIgnoreCase(STATUS) || "ERROR".equalsIgnoreCase(STATUS) || "FAIL".equalsIgnoreCase(STATUS) || "ERROR3DS".equalsIgnoreCase(STATUS))
                    {
                        ...
                        dbBuffer.append("',authorization_code='"+amount+"',customerIp='" + requestIp + "',customerIpCountry='" + functions.getIPCountryShort(requestIp) + "' ,remark='" +message+ "' where trackingid = " + splitTrackingId);
                        transactionlogger.error("Update Query---" + dbBuffer);
                        Database.executeUpdate(dbBuffer.toString(), con);
                        entry.actionEntryForCommon(splitTrackingId, amount, ActionEntry.ACTION_AUTHORISTION_FAILED, ActionEntry.STATUS_AUTHORISTION_FAILED, comm3DResponseVO, auditTrailVO, requestIp);
                    }
                    else {
                        //pending
                ...
                else
                {
                    ...
                    {
                        transactionlogger.error("inside EPFrontEndServlet transactionStatus " + transactionStatus);
                        status      = "pending";
...
//                asynchronousMailService.sendEmail(MailEventEnum.PARTNERS_MERCHANT_SALE_TRANSACTION, String.valueOf(splitTrackingId), status, message, billingDesc);

//                AsynchronousSmsService smsService = new AsynchronousSmsService();
//                smsService.sendSMS(MailEventEnum.PARTNERS_MERCHANT_SALE_TRANSACTION, String.valueOf(splitTrackingId), status, message, billingDesc);
                ...
                commonValidatorVO.setBankCode(bankcode);
                commonValidatorVO.setBankDescription(bankDescription);
        ...
        catch (Exception e)
        {
            ...
            PZExceptionHandler.raiseAndHandleDBViolationException("EPFrontEndServlet.java", "doService()", null, "Transaction", null, PZDBExceptionEnum.INCORRECT_QUERY, null, e.getMessage(), e.getCause(), toid, null);
        }
        finally
        {
