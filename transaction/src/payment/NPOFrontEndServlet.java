...
public class NPOFrontEndServlet extends HttpServlet
{
    ...
    public void doService(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        try
        {
            ...
            redirectMethod          = transactionDetailsVO1.getRedirectMethod();
            ...
            ccnum        = transactionDetailsVO1.getCcnum();
            ...
            if ("N".equals(isService)||transactionDetailsVO1.getTransactionType().equals("PA"))
            {
                transType = "auth";
            }
            transactionLogger.error("TransactionType---------> "+trackingId+" "+transactionDetailsVO1.getTransactionType());

            try
            {
                if (PZTransactionStatus.AUTH_STARTED.toString().equals(dbStatus))//if authstarted
                {
                    ...
                    TrustPayOppwaPaymentGateway trustPayOppwaPaymentGateway = new TrustPayOppwaPaymentGateway(accountId);
                    entry.actionEntryFor3DCommon(trackingId, amount, ActionEntry.ACTION_3D_CONFIRMATION_STARTED, ActionEntry.STATUS_3D_CONFIRMATION, transRespDetails, commRequestVO, auditTrailVO, null);
                        ...
                        else if (fromType.equals("trustpay"))
                        {
                            transRespDetails = (Comm3DResponseVO) trustPayOppwaPaymentGateway.processInquiry(commRequestVO);
                        }
                        else
                        {
                    ...
                    {
                        notificationUrl     = transactionDetailsVO1.getNotificationUrl();
                        ...
                        if ("sale".equals(transType))
                        {
                            ...
                            dbBuffer.append("update transaction_common set captureamount='" + amount + "',paymentid='" + transactionId + "',status='capturesuccess',eci='" + eci + "'" + ",successtimestamp='" + functions.getTimestamp()+"'");
                ...
                transactionDetailsVO1.setCcnum(ccnum);


              //  transactionLogger.error("trust pay frontend before  sending notification---" + notificationUrl);
                if((functions.isValueNull(notificationUrl) || functions.isValueNull(merchantDetailsVO.getNotificationUrl()) ) && sendnotification) //&& ("3D".equals(merchantDetailsVO.getTransactionNotification())||"Both".equals(merchantDetailsVO.getTransactionNotification()))
                {
