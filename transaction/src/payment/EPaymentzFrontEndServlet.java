 ...
 */
public class EPaymentzFrontEndServlet extends HttpServlet
{
    private static TransactionLogger transactionlogger = new TransactionLogger(EPaymentzFrontEndServlet.class.getName());
    ...
    public void doGet(HttpServletRequest req,HttpServletResponse res) throws ServletException, IOException
    {
        ...
        {
            transactionlogger.error("Entering EPaymentzFrontEndServlet ......",e);
        }
    }
    ...
    {
        transactionlogger.error("Entering EPaymentzFrontEndServlet ......");
        ...
        try
        {
            transactionlogger.error("..... EPaymentzFrontEndServlet ......"+trackingId);
            /*if(functions.isValueNull(req.getParameter("paymentOption")))
            {
                paymentOption = req.getParameter("paymentOption");
            }
            if(functions.isValueNull(req.getParameter("MerchantorderId")))
            {
                MerchantorderId = req.getParameter("MerchantorderId");
            }
            if(functions.isValueNull(req.getParameter("orderAmount")))
            {
                AMOUNT = req.getParameter("orderAmount");
            }
            if(functions.isValueNull(req.getParameter("VendorOrderId")))
            {
                VendorOrderId = req.getParameter("VendorOrderId");
            }
            if(functions.isValueNull(req.getParameter("status")))
            {
                resStatus = req.getParameter("status");
            }
            if(functions.isValueNull(req.getParameter("txtDate")))
            {
                txtDate = req.getParameter("txtDate");
            }
            ...
            transactionlogger.error(" paymentOption--->" + paymentOption);
            transactionlogger.error(" MerchantorderId--->" + MerchantorderId);
            transactionlogger.error(" AMOUNT--->" + AMOUNT);
            transactionlogger.error(" VendorOrderId--->" + VendorOrderId);
            transactionlogger.error(" resStatus--->" + resStatus);
            transactionlogger.error(" txtDate--->" + txtDate);
            ...
            if (transactionDetailsVO != null)
            {
                ...
                auditTrailVO.setActionExecutorName("EPFrontEndServlet");
                ...
                if (PZTransactionStatus.AUTH_STARTED.toString().equals(dbStatus))
                {
                    ...
                    commResponseVO  = (CommResponseVO) easyPaymentzPaymentGateway.processQuery(trackingId, requestVO);
                    ...
                    {    notificationUrl    = transactionDetailsVO.getNotificationUrl();
                        ...
                        dbBuffer.append("update transaction_common set captureamount='" + amount + "',paymentid='" + TXN_ID + "',templateamount='" + tmpl_amt + "',templatecurrency='" + tmpl_currency +  "',rrn='" + "" +"',status='capturesuccess'" + " ,successtimestamp='" + functions.getTimestamp());
                    ...
                    {
                        transactionlogger.error("inside EasyPayemetz failed --->");
                        ...
                        status      = "Pending";
                        message     = "Pending";
                        transactionlogger.error("inside EasyPaymentz pending condition --->"+STATUS+"--"+message);
                    }
