 ...
 */
public class PayUGPostWait extends PzServlet
{
    private static Logger logger = new Logger(PayUGPostWait.class.getName());
    private static TransactionLogger transactionLogger = new TransactionLogger(PayUGPostWait.class.getName());
    ...
    {
        logger.debug("Entering doService of PayUGPostWait");
        transactionLogger.debug("Entering doService of PayUGPostWait");
        ...
        try
        {
            ...
            if (version.equals("2"))
            {
                ...
                transactionLogger.debug("condition is true");
            ...
            else
            {
                ...
                transactionLogger.debug("condition is false");
            ...
            transactionLogger.debug("verify          "+verify);
            /*if (!verify)
            {
            ...
            else
            {
                ...
                if (!triesOver)
                {
                    ...
                    //otherdetails.put("RETRYBUTTON", "<input type=\"button\" value=\"Retry \" onClick=\"document.location.href='" + PAYPROCESS + "?toid=" + memberId + "&totype=pz&description=" + URLEncoder.encode(req.getParameter("DESCRIPTION")) + "&orderdescription=" + req.getParameter("ORDER_DESCRIPTION") + "&amount=" + amt.toString() + "&redirecturl=" + req.getParameter("REDIRECTURL")+ "&paymenttype="+req.getParameter("PAYMODEID") + "&cardtype="+req.getParameter("CARDTYPEID")+"&checksum=" + Checksum.generateChecksumV1(memberId, "pz", amt.toString(), req.getParameter("DESCRIPTION"), req.getParameter("REDIRECTURL"), clkey, rs.getString("checksumalgo"))  + templatevar + "&TMPL_VERSION=" + version + "'\">");
                    //ToDo Retry button should be well formed
                }
            ...
            try
            {
                ...
                try
                {
                    partnerLogo = merchants.getPartnerLogo(memberId);
                }
                catch(Exception e)
                {
