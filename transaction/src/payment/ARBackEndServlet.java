 ...
 */
public class ARBackEndServlet extends HttpServlet
{
    //private static TransactionLogger transactionlogger  = new TransactionLogger(ARBackEndServlet.class.getName());
    private static AirPayTransactionLogger transactionlogger  = new AirPayTransactionLogger(ARBackEndServlet.class.getName());
    ...
    public void doGet(HttpServletRequest req,HttpServletResponse res) throws ServletException, IOException
    {
        ...
        {
            transactionlogger.error("Entering ARBackEndServlet ......",e);
        }
    }
    ...
    {
        transactionlogger.error("....Entering ARBackEndServlet ......");
        ...
        {
            responseMsg.append(strJSON);
        }
        strJSON = responseMsg.toString();
        transactionlogger.error("-----AirPayBackEndServlet  response-----" + responseMsg);

        try
        {
            if(req.getParameter("TRANSACTIONID") != null){
                trackingId = req.getParameter("TRANSACTIONID");
            }

            transactionlogger.error("trackingId-->"+trackingId);


            if(functions.isValueNull(strJSON) && functions.isJSONValid(strJSON)){
                JSONObject postdataJSON   = new JSONObject(strJSON.toString());
                transactionlogger.error("postdataJSON" + postdataJSON);

                if (postdataJSON.has("TRANSACTIONID"))
                {
                    trackingId = postdataJSON.getString("TRANSACTIONID");
                    transactionlogger.error("trackingId-->"+trackingId);
                }

                if (postdataJSON.has("APTRANSACTIONID"))
                {
                    transactionid = postdataJSON.getString("APTRANSACTIONID");
                }
                ...
                if (postdataJSON.has("TRANSACTIONSTATUS"))
                {
                    txnStatus = postdataJSON.getString("TRANSACTIONSTATUS");
                }
                if (postdataJSON.has("MESSAGE"))
                {
                    message = postdataJSON.getString("MESSAGE");
                }
                if (postdataJSON.has("CUSTOMERVPA"))
                {
                    CUSTOMERVPA = postdataJSON.getString("CUSTOMERVPA");
                }
                if (postdataJSON.has("ap_SecureHash"))
                {
                    CUSTOMERVPA = postdataJSON.getString("ap_SecureHash");
                }
            }
            ...
            if (transactionDetailsVO != null)
            {
                ...
                auditTrailVO.setActionExecutorName("BackEnd");
                auditTrailVO.setActionExecutorId(toid);

                AirpayPaymentGateway airpayPaymentGateway     = new AirpayPaymentGateway(accountId);
                ...
                if (PZTransactionStatus.AUTH_STARTED.toString().equals(dbStatus))
                {
                    ...
                    commResponseVO  = (CommResponseVO) airpayPaymentGateway.processQuery(trackingId, requestVO);
                    ...
                    else if("Failed".equalsIgnoreCase(STATUS))
                    {
                        ...
                        transactionlogger.error("inside Payu pending condition --->"+STATUS+"--"+message);
                    }
