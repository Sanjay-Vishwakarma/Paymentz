...
public class SendMailToBank extends HttpServlet
{
    ...
    public void doPost(HttpServletRequest request,HttpServletResponse response) throws IOException,ServletException
    {
        ...
        try
        {
            ...
            {
                logger.debug("Delete.......");
                logger.debug("Delete Status....."+request.getParameter(actionVO.getActionCriteria().split("_")[0]+"_status"));

                consolidatedAppStatus=ConsolidatedAppStatus.getEnum(request.getParameter(actionVO.getActionCriteria().split("_")[0]+"_status"));
                logger.debug("Delete consolidatedAppStatus......."+consolidatedAppStatus);
                isDeleted = applicationManager.deleteConsolidated_application(actionVO.getActionCriteria().split("_")[0]);
                logger.debug("Delete isDeleted......."+isDeleted);
                if (isDeleted){
                    request.setAttribute("message","consolidated application deleted successfully.");
                }
                /*ConsolidatedApplicationVO consolidatedApplicationVOstatus = new ConsolidatedApplicationVO();
                consolidatedApplicationVOstatus.setStatus(BankApplicationStatus.VERIFIED.toString());*/

                saved = applicationManager.updateConsolidatedAppHistory(consolidatedAppStatus, actionVO.getActionCriteria().split("_")[0]);
                logger.debug("Updated......."+isUpdated);
            ...
            else /*if(functions.isValueNull(request.getParameter("action")) && request.getParameter("action").contains("_Download"))*/
            {
                ...
                String filePath = APPLICATION_ZIP_PATH +memberid+ "/" + fileName;
