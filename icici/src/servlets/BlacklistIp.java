...
public class BlacklistIp extends HttpServlet
{
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
    {
        ...
        try
        {
            listOfall = blacklistManager.getBlockedipForAdmin(memberId, allIP, selectIpVersion, actionExecutorId, actionExecutorName, reason, remark, paginationVO);
        ...
        try
        {
            if ("upload".equalsIgnoreCase(action))
            {
                ...
                if (functions.isValueNull(selectIpVersion) && functions.isValueNull(allIP))
                {
                    ...
                    else
                    {
                        if (!functions.isValueNull(memberId))
                        {
                            blacklistManager.insertIPForAdmin(allIP, selectIpVersion, actionExecutorId, actionExecutorName, reason, remark);
                        }
                        else
                        {
                            blacklistManager.insertIPForAdmin(memberId, allIP, selectIpVersion, actionExecutorId, actionExecutorName, reason, remark);
                        }
                        msg = "IP Address Blocked Successfully.";
                        req.setAttribute("msg", msg);
                    }
                    listOfall = blacklistManager.getBlockedipOnes(memberId, allIP, selectIpVersion, reason, remark, paginationVO);
                }
                else
                {
            ...
            if ("delete".equalsIgnoreCase(delete))
            {
                ...
                listOfall = blacklistManager.getBlockedips(memberId, allIP, selectIpVersion,reason,remark, paginationVO);
            }
