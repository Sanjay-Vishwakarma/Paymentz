...
<head>
  ...
  <script>
    ...
    function getSettlementCurrency(ctoken)
    {
      ...
      var parent_bankWireId = document.getElementById("parent_bankWireId").value;
     /* if (bankWireId == null || bankWireId=='' || bankWireId=='null'){
        alert('Please select bankwire');
        return false;
      }*/
//      else{
        document.f1.action = "/icici/servlet/ConsolidatedGetSettlementCurrencyDetails?ctoken=" + ctoken + "&bankwireid=" + bankWireId + "&parent_bankWireId=" + parent_bankWireId;
        document.f1.submit();
//      }
    }
    function getPaymodOrCardId(ctoken)
    {
      var cardtypeid = document.getElementById("cardtypeid").value;
      var paymodid = document.getElementById("paymodid").value;
     /* if (bankWireId == null || bankWireId=='' || bankWireId=='null'){
        alert('Please select bankwire');
        return false;
      }*/
//      else{
        document.f1.action = "/icici/servlet/ConsolidatedMerchantPayoutReport?ctoken=" + ctoken + "&cardtypeid=" + cardtypeid + "&paymodid=" + paymodid;
        document.f1.submit();
//      }
    }
  ...
  <title> Consolidated Payout Report</title>
</head>
<body>
<%
  ...
  if (com.directi.pg.Admin.isLoggedIn(session))
  {
    ...
    List<String> parentBankWiresList = bankManager.getparentBankwireIdForRandomCharges();
    String bankWireId=request.getParameter("bankwireid");
    String parent_bankWiresId=request.getParameter("parent_bankWireId");
    String cardtypeid=request.getParameter("cardtypeid");
    String paymodeid=request.getParameter("paymodid");
    String accountID = request.getAttribute("accountId")!=null ? String.valueOf(request.getAttribute("accountId")) : "";
    System.out.println("accountID============"+accountID);
    List<String> cardtypeidList = null;
    List<String> paymodeidList = null;
    if (functions.isValueNull(accountID))
    {
      cardtypeidList = bankManager.getCardtypeIdByAccountId(accountID);
      paymodeidList = bankManager.getPaymodeIdByAccountId(accountID);
    }
    ...
    HashMap<String,List<TerminalVO>> getListOfData= (HashMap<String, List<TerminalVO>>) request.getAttribute("getListOfData");
    ...
    session.setAttribute("getListDetails",getListOfData);
...
<div class="row">
  <div class="col-lg-12">
    <div class="panel panel-default" >
      ...
      <form action="/icici/servlet/ConsolidatedMerchantPayoutReport?ctoken=<%=ctoken%>" method="post" name="f1">
        <input type="hidden" value="<%=ctoken%>" name="ctoken">
        <input type="hidden" value="" name="pgtypeid">
        <input type="hidden" value="" name="accid">
            ...
            <td>
              <table border="0" cellpadding="5" cellspacing="0" width="100%" align="center">
                ...
                <tr>
                  ...
                  <td width="3%" class="textb">:</td>
                  <td width="12%" class="textb">
                    <%--<input name="parent_bankwireId" id="parent_bankwireId" &lt;%&ndash;value="<%=parent_bankwireId%>"&ndash;%&gt; class="txtbox" autocomplete="on">--%>
                      <select name="parent_bankWireId" id="parent_bankWireId" class="txtbox" style="width: 165px"  onchange=" return getSettlementCurrency('<%=ctoken%>')">
                        <option value="" selected></option>
                        <%
                          if (parentBankWiresList!=null){
                          for (String parentBankID : parentBankWiresList)
                          {
                            String isSelected = "";
                            if (parentBankID.equalsIgnoreCase(parent_bankWiresId))
                              isSelected = "selected";
                        %>
                        <option value="<%=parentBankID%>" <%=isSelected%>> <%=parentBankID%></option>
                        <%
                          }
                          }
                        %>
                ...
                <tr>
                  <td width="4%" class="textb">&nbsp;</td>
                  <td width="11%" class="textb">Cardtype Id</td>
                  <td width="3%" class="textb">:</td>
                  <td width="12%" class="textb">
                    <%--<input name="parent_bankwireId" id="parent_bankwireId" &lt;%&ndash;value="<%=parent_bankwireId%>"&ndash;%&gt; class="txtbox" autocomplete="on">--%>
                      <select name="cardtypeid" id="cardtypeid" class="txtbox" style="width: 165px" <%-- onchange=" return getPaymodOrCardId('<%=ctoken%>')"--%>>
                        <option value="" selected></option>
                        <%
                          if (cardtypeidList!=null){
                          for (String cardTypeId : cardtypeidList)
                          {
                            String isSelected = "";
                            if (cardTypeId.equalsIgnoreCase(cardtypeid))
                              isSelected = "selected";
                        %>
                        <option value="<%=cardTypeId%>" <%=isSelected%>> <%=cardTypeId%></option>
                        <%
                          }
                          }
                        %>
                ...
                <tr>
                  <td width="4%" class="textb">&nbsp;</td>
                  <td width="11%" class="textb">Paymode Id</td>
                  <td width="3%" class="textb">:</td>
                  <td width="12%" class="textb">
                    <%--<input name="parent_bankwireId" id="parent_bankwireId" &lt;%&ndash;value="<%=parent_bankwireId%>"&ndash;%&gt; class="txtbox" autocomplete="on">--%>
                      <select name="paymodid" id="paymodid" class="txtbox" style="width: 165px" <%-- onchange=" return getPaymodOrCardId('<%=ctoken%>')"--%>>
                        <option value="" selected></option>
                        <%
                          if (paymodeidList!=null){
                          for (String paymodeID : paymodeidList)
                          {
                            String isSelected = "";
                            if (paymodeID.equalsIgnoreCase(paymodeid))
                              isSelected = "selected";
                        %>
                        <option value="<%=paymodeID%>" <%=isSelected%>> <%=paymodeID%></option>
                        <%
                          }
                          }
                        %>
      ...
      <div style="width: 100%">
        <form name="TerminalDetails" action="/icici/servlet/ConsolidatedMerchantPayoutReport?ctoken=<%=ctoken%>" method="post">
          ...
          <input type="hidden" name="parent_bankWireId" value="<%=parent_bankWiresId%>">
          <%
          ...
          <div style="overflow-x:auto;overflow-y: hidden;width: 100%;">
            <table align=center style="" border="1" class="table table-striped table-bordered table-hover table-green dataTable">
              ...
              <tr>
                ...
                <% if (functions.isValueNull(parent_bankWiresId)){%>
                <td width="" valign="middle" align="center" class="th0">Parent Bank Wire Id</td>
                <td width="" valign="middle" align="center" class="th0">Bank Wire Id</td>
                <% } %>
              ...
              <tr>
                ...
                <% if (functions.isValueNull(parent_bankWiresId)){%>
                <td align="center" class="tr0"><%=terminalVO.getParentBankWireId()!=null?terminalVO.getParentBankWireId():"-"%><input type="hidden" name="parentWireId_<%=terminalVO.getTerminalId()%>" value="<%=terminalVO.getParentBankWireId()%>"></td>
                <td align="center" class="tr0"><%=terminalVO.getWireId()!=null?terminalVO.getWireId():"-"%><input type="hidden" name="wireId_<%=terminalVO.getTerminalId()%>" value="<%=terminalVO.getWireId()%>"></td>
                <%}%>
            ...
            <div style="text-align: center">
              <button type="button" name="action" value="submit" class="buttonform" onclick="return Submit();" style="display: inline-block;">
                <i class="fa fa-sign-in"></i>
                &nbsp;&nbsp;Submit
              </button>
            </div>
