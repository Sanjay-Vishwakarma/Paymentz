...
public class AgentDashboard extends HttpServlet
{
    ...
    private static String[] colorPallet = new String[] {"#68c39f", "#edce8c", "#b4c0c0", "#e8a451","#e8e85a", "#4a525f", "#99e6ff", "#b399ff", "#6fc51b", "#ff99e6", "#ff9999", "#3b76c7", "#d689c0", "#b93a71", "#990000", "#54ce28", "#68c39f", "#edce8c", "#4A2CC9", "#C92C5C","#68c39f"};
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
    {
        ...
        try
        {
           ...
           // System.out.println("AgentId here----->"+agentId);
                ...
                prepareStatusChartData(agentId, lastYearTime, conn, session, statusChartString);
                prepareSalesChartData(agentId, lastYearTime, conn, session, salesChartString);
                prepareRefundChartData(agentId, lastYearTime, conn, session, refundChartString);
                prepareChargeBackChartData(agentId, lastYearTime, conn, session, chargeChartString);
                prepareFraudChartData(agentId, lastYearTime, conn, session,fraudChartString);
                ...
                BigDecimal salesCount = totalSales(agentId, lastYearTime, conn);

                List<BigDecimal> totalHashtable = totalChargebackCaptureRefund(agentId, lastYearTime, conn);
            ...
            RequestDispatcher rd = req.getRequestDispatcher("/agentDashboard.jsp?ctoken=" + user.getCSRFToken());
            rd.forward(req, res);
        }
        ...
        {
            logger.error("SQLException In AgentDashboard ",e);
        }
        finally
        {
            if (conn != null)
    ...
    private void prepareStatusChartData(int agentId, String lastYearTime, Connection conn, HttpSession session,StringBuffer chartString) throws SystemError, SQLException
    {
        logger.error("prepare Status Chart Data for agent");
       ...
       // System.out.println("sMemberList here----->"+sMemberList);
        ...
        logger.error("AgentDashBoard query for prepareStatusChartData method+++ "+query);
        logger.debug("===query=="+query.toString());

        countquery = new StringBuffer("select status, SUM(count) as count from ( " + query + ")as temp  group by status order by count");
        logger.error("AgentDashBoard Countquery for prepareStatusChartData method+++ "+countquery);
    ...
    private void prepareSalesChartData(int agentId, String lastYearTime, Connection conn, HttpSession session,StringBuffer chartString) throws SystemError, SQLException
    {
    ...
    private void prepareRefundChartData(int agentId, String lastYearTime, Connection conn, HttpSession session,StringBuffer chartString) throws SystemError, SQLException
    {
    ...
    private void prepareChargeBackChartData(int agentId, String lastYearTime, Connection conn, HttpSession session,StringBuffer chartString) throws SystemError, SQLException
    {
    ...
    private void prepareFraudChartData(int agentId, String lastYearTime, Connection conn, HttpSession session,StringBuffer chartString) throws SystemError, SQLException
    {
    ...
    private BigDecimal totalSales(int agentId, String lastYearTime, Connection con) throws SystemError,SQLException
    {
        String tablename = "";
        String fields = "";
        int salesCount = 0;
    ...
    private List<BigDecimal> totalChargebackCaptureRefund(int agentId, String lastYearTime, Connection connection) throws SystemError,SQLException
    {
        String tablename = "";
        String fields = "";
        AgentFunctions pAgent = new AgentFunctions();
    ...
    public List<String> perCurrency(int agentId, String lastYearTime, Connection connection) throws SystemError, SQLException
    {
        String tablename = "";
        String fields = "";
        ...
        while(i.hasNext())
        {
            tablename = (String)i.next();
            //fields = "sum(captureamount) as capture, sum(refundamount) as refund, sum(chargebackamount) as chargeback";
            query.append("select currency as curr from " + tablename + " where");
            query.append(" toid IN(" + sMemberList + ")");
            query.append(" and dtstamp >='" + lastYearTime + "'");
            if(i.hasNext())
                query.append(" UNION ");
        ...
        if(query.length()>0)
        {
            countquery = new StringBuffer("select curr from ( " + query + ") as temp group by curr");
            logger.debug("Countquery in perCurrency-------->"+countquery);
            ResultSet rs = Database.executeQuery(countquery.toString(), connection);
