...
public class DoReverseTransaction extends HttpServlet
{
   ...
   // private static TransactionLogger transactionLogger = new TransactionLogger(DoReverseTransaction.class.getName());
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
    {
        ...
        try
        {
            reason          = ESAPI.validator().getValidInput("reason", req.getParameter("reason"), "Description", 100, false);
            ...
            bankId = functions.isValueNull(req.getParameter("bankname")) ?  req.getParameter("bankname") : "";
        ...
        {
            logger.error("Exception In DoReverseTransaction INPUT", e);
        }
                ...
                if (gatewayType != null && CUPPaymentGateway.GATEWAY_TYPE.equals(gatewayType))
                {
                    if (!trackingId.trim().equals("") && !transId.trim().equals("") && !reason.trim().equals(""))
                    {
                        ...
                        if(functions.isValueNull(transactionDetailsVO.getNotificationUrl())){
                            refundRequest.setNotificationURL(transactionDetailsVO.getNotificationUrl());
                        }else{
                            refundRequest.setNotificationURL("");
                        }
                ...
                else //Generic Reverse for all gateways
                {
                    ...
                    if(refundChecker.isRefundAllowed(memberId))
                    {
                        ...
                        else
                        {
                            ...
                            if (isAllowed)
                            {
                                refundRequest.setBankId(bankId);
                                refundResponse  = paymentProcess.refund(refundRequest);
                                responseStatus  = refundResponse.getStatus();
                                paymentURL      = refundResponse.getPaymentURL();
                                logger.error("responseStatus >>>>>>>>> "+responseStatus);
                                logger.error("paymentURL >>>>>>>>> "+paymentURL);
