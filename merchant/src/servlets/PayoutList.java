    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
    {
        ...
        if(checkBalance.equalsIgnoreCase("checkBalance"))
        {
            try
            {
               ...
               String payoutAmount   = getPayoutAmount(todaysStartTime, endStartTime, merchantId);
               String totalPayoutAmount   = merchantDAO.getTotalPayoutAmount(merchantId);
                logger.debug("capt= "+capt+" payoutAmout = "+payoutAmount + ", totalPayoutAmount = " + totalPayoutAmount);

                req.setAttribute("payoutsumm", capt);
                req.setAttribute("payoutAmount", payoutAmount);
                req.setAttribute("totalPayoutAmount", totalPayoutAmount);
            }
            catch (Exception e)
            {
        ...
        try{
            ...
            logger.error("PAYOUTLIST QUERY+++++  "+query.toString());
   ...
   public String getPayoutBalanceAmount(long todaysStartTimeInSecs,long endStartTimeInSecs,String memeberId){
    ...
    public String getPayoutAmount(long todaysStartTimeInSecs,long endStartTimeInSecs,String memeberId){
        ...
        try{
            ...
            psmt.setString(3, String.valueOf(endStartTimeInSecs));
            resultSet = psmt.executeQuery();
            logger.debug("getPayoutAmount Query " + psmt);
