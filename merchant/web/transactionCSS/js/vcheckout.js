...
var EmptyEmail, InvalidEmail, EmptyAddress,InvalidAddress, EmptyCity, EmptyCountry, EmptyState, EmptyZip, EmptyBirhtdate;
...
var landingTab;
function loadFromBody(paymentmethodLabel, paymentmethod, cardtype, cardtypeDisplayLabel, error, paymentList, personalDetailDisplay, personalDetailValidation, payType, payTypeLabel, terminalMapSize)
{
    console.log("in load from body", paymentmethodLabel, paymentmethod, cardtype, cardtypeDisplayLabel, error, paymentList, personalDetailDisplay, personalDetailValidation, payType, payTypeLabel, terminalMapSize);
...
function PaymentOptionHideShow(method, engLabel)
{
    ...
    all_options = [
        ...
        'Net-Banking', 'Wallets', 'UPI' , 'CRYPTO', 'ROMCARD', 'TOJIKA', 'CupUpi','BITCOIN'  /*, 'CreditCardsIndia' , 'DebitCardsIndia',*/
    ];
    ...
    else if (pay_methods == "CupUpi")
    {
        typeLabel = label;
        showTab(0, pay_methods);
        CupUpiHideShow('CupUpi');
    }
    else if (pay_methods == "ACH" || pay_methods == "ROMCARD" || pay_methods == "TOJIKA")
    {
        typeLabel = label;
        showTab(0, pay_methods);
    }
...
{
    console.log("in crypto hide show ",method,displayLabel);
    typeLabel = displayLabel;
    // $("#CryptoOption").hide();
    $("#" + method).addClass('active');
    document.getElementById('payMethodDisplay').innerText = typeLabel;
...
{
    console.log("in BITCOIN hide show ",method,displayLabel);
    typeLabel = displayLabel;
    $("#BitCoinOption").hide();
    $("#" + method).addClass('active');
    document.getElementById('payMethodDisplay').innerText = typeLabel;
...
function CupUpiHideShow(method)
{
    availableCardType.push("VISA");
    availableCardType.push("MC");
    availableCardType.push("AMEX");
...
function pay()
{
    if ($("#" + pay_methods).find(".active")[1] != undefined)
    {
        ...
        else
        {
            ...
            {
                $("#" + formname).attr('action', '/transaction/SingleCallVirtualCheckout');
                $("#" + formname).submit()
            }
...
function nextPrev(n)
{
    ...
    console.log("in begining ", currentTab);
    if ($("#" + pay_methods).find(".active")[1])
    {
...
function validateForm(formname)
{
    ...
    for (i = 0; i < y.length; i++)
    {
        ...
        else if (addressDisplayFlag == "N" && ( x[currentTab].id.indexOf("cardinfo") > -1)&& addressValidation == "N")
        {
            ...
            if(y[i].id.indexOf("amount") > -1 ){
                if (validateAmount(y[i].value, document.getElementById(y[i].id).parentElement.id))
                {
                    console.log("in first if amount")
                    valid = true;
                }
                else
                {
                    console.log("in first else amount");
                    $('#cardinfo').animate({
                        scrollTop: (0, 0)
                    }, 500);
                    y[i].className += " invalid";
        ...
        {
            console.log("in else", y[i].id,x[currentTab].id)
...
function expiryCheck(exp, divID)
{
    ...
    if (length >= 4)
    {
        ...
        else
        {
            console.log(val);
            valid = true;
            // if(!(val.indexOf("/")) > -1){ console.log("value - " , val , val.slice(0, 2) + "/" + val.slice(2,4))
...
function validateAmount(inputText, divID) {
    ...
    else if (!(inputText.match(AmountRegex)))
    {
        error = "Invalid Amount";
        valid = false;
    }
    else if(inputText <= 0) {
        error = "Amount cannot be less than or equal to 0";
        valid = false;
    }
    else
    {
...
function validateAddress(inputText, divID)
{
    ...
    //console.log("in validate address",regex.test(inputText))
...
/*function funcCheck(cNo , divID){
 console.log(cNo);
 var cardNo =document.getElementById(cNo).value;
 var card_type =document.getElementById("payMethod").innerText;
 console.log(cardNo , card_type , divID)
 return isValidCard(cardNo , card_type ,divID)
 }*/

/*function isValidCard(ccNum, cardType ,divID){
 //ccNum.trim();
 console.log(cardType.split(" > ")[0])
 var cardType = cardType.split(" > ")[0];
 var length = ccNum.length;
 var error = "";
 var type = "";
 var valid;
 if (cardType == "VISA" || cardType == "Visa Card")
 {
 if(!ccNum.startsWith("4")){
 error = "Invalid Visa Card";
 valid = false;
 }
 else if(length != 13 && length != 16 && length != 19){ console.log( length ,length != 13 , length != 16 , length != 19 )
 error = "Invalid Visa Card Number ( Maxlength 19 )";
 valid = false;
 }
 else{
 valid = true;
 ...
 else if (cardType == "Master Card" ||  cardType == "MC")
 {
 if(!(ccNum.startsWith("51") || ccNum.startsWith("52") || ccNum.startsWith("53") || ccNum.startsWith("54") || ccNum.startsWith("55") || ccNum.startsWith("22") ||
 ...
 error = "Invalid Master Card Number (Maxlength 16)";
 valid = false;
 }
 else{
 valid = true;
 ...
 else if (cardType == "Diners Club" || cardType == "DINER")
 {
 if(!(ccNum.startsWith("300") || ccNum.startsWith("301") || ccNum.startsWith("302") || ccNum.startsWith("303") || ccNum.startsWith("304")
 || ccNum.startsWith("305") || ccNum.startsWith("36") || ccNum.startsWith("54") || ccNum.startsWith("55")))
 {
 error = "Invalid DINERS Card";
 valid = false;
 }
 else if(length != 14 && length != 16){ console.log( length ,length != 14 , length != 16)
 error = "Invalid DINERS Card Number (Maxlength 16)";
 valid = false;
 }
 else{
 valid = true;
 ...
 else if (cardType == "American Express" || cardType == "AMEX")
 {
 ...
 error = "Invalid AMEX Card Number (Maxlength 15)";
 valid = false;
 }
 else{
 valid = true;
 ...
 else if (cardType == "Discover" || cardType == "DISC")
 {
 if(!(ccNum.startsWith("6011") || ccNum.startsWith("622") || ccNum.startsWith("644") || ccNum.startsWith(" 645") || ccNum.startsWith("646") || ccNum.startsWith("647") || ccNum.startsWith("648") || ccNum.startsWith("649") || ccNum.startsWith("65"))){
 error = "Invalid DISC Card";
 valid = false;
 }
 else if(length != 16 && length != 19){
 error = "Invalid DISC Card Number (Maxlength 16)";
 valid = false;
 }
 else{
 valid = true;
 ...
 {
 if(!(ccNum.startsWith("35"))){
 error = "Invalid JCB Card";
 valid = false;
 }
 else if(length != 16 && length != 19){
 error = "Invalid JCB Card Number (Maxlength 19)";
 valid = false;
 }
 else{
 valid = true;
 ...
 else if (cardType == "Maestro" || cardType == "MAESTRO")
 {
 if(!(ccNum.startsWith("5018") || ccNum.startsWith("5020") || ccNum.startsWith("5038") || ccNum.startsWith("5893") || ccNum.startsWith("6304") || ccNum.startsWith("6759") || ccNum.startsWith("6761") || ccNum.startsWith("6762") || ccNum.startsWith("6763"))){
 error = "Invalid MAESTRO Card";
 valid = false;
 }
 else if(length != 16 && length != 19){
 error = "Invalid MAESTRO Card Number (Maxlength 19)";
 valid = false;
 }
 else{
 valid = true;
 ...
 else if (cardType == "Insta Payment" || cardType == "INSTAPAYMENT")
 {
 if(!(ccNum.startsWith("637") || ccNum.startsWith("638") || ccNum.startsWith("639"))){
 error = "Invalid INSTAPAYMENT Card";
 valid = false;
 }
 else if(length != 16){
 error = "Invalid INSTAPAYMENT Card Number (Maxlength 16)";
 valid = false;
 }
 else{
 valid = true;
...
function isValidCardCheck(cardno, divID ,triggered)
{
    ...
    /* if(triggered){
     console.log("in if triggered", triggered)
     }
     else{
     console.log("in else no trigger")
     }*/
    ...
    else if (ccNum.indexOf("6011") === 0 || ccNum.indexOf("622") === 0 || ccNum.indexOf("644") === 0 || ccNum.indexOf(" 645") === 0 || ccNum.indexOf("646") === 0 || ccNum.indexOf("647") === 0 || ccNum.indexOf("648") === 0 || ccNum.indexOf("649") === 0 || ccNum.indexOf("65") === 0 )
    {
        if (length != 16 && length != 19)
        {
            error = InvalidDISC;
    ...
    else if (ccNum.indexOf("5018") === 0 || ccNum.indexOf("5020") === 0 || ccNum.indexOf("5038") === 0 || ccNum.indexOf("5893") === 0 || ccNum.indexOf("6304") === 0 || ccNum.indexOf("6759") === 0 || ccNum.indexOf("6761") === 0 || ccNum.indexOf("6762") === 0 || ccNum.indexOf("6763") === 0 )
    {
        if (length != 16 && length != 19)
        {
    ...
    if (valid == true)
    {
        ...
        if (pay_methods == 'CreditCards')
        {
            ...
            if ($("#CreditCards").find("#" + type)[0])
            {
                ...
                {
                    var inpLen = document.getElementById("CreditCards").querySelectorAll("#" + type).length;
                    for (var i = 0; i < inpLen; i++)
                    {
                        console.log("inp value = ",document.getElementById("CreditCards").querySelectorAll("#" + type)[i].value)
                        if (document.getElementById("CreditCards").querySelectorAll("#" + type)[i].value.indexOf(document.getElementById('currencydropdown').value) > -1)
                        {
                            val = document.getElementById("CreditCards").querySelectorAll("#" + type)[i].value;
                            break;
                        }
                    }
                    if(!val){
                        error_msg("This card does not support the requested currency", divID);
                        valid = false;
                    }
                }
                else
                {
                    val = $("#CreditCards").find("#" + type)[0].value ;
                }
        ...
        if (pay_methods == 'DebitCards')
        {
            ...
            {
                var val;
                if(document.getElementById('currencydropdowndc'))
                {
                    var inpLen = document.getElementById("DebitCards").querySelectorAll("#" + type + "_DC").length;
                    for (var i = 0; i < inpLen; i++)
                    {
                        console.log("inp value = ",document.getElementById("DebitCards").querySelectorAll("#" + type+ "_DC")[i].value)
                        if (document.getElementById("DebitCards").querySelectorAll("#" + type + "_DC")[i].value.indexOf(document.getElementById('currencydropdowndc').value) > -1)
                        {
                            val = document.getElementById("DebitCards").querySelectorAll("#" + type + "_DC")[i].value;
                            break;
                        }
                    }
                    if(!val){
                ...
                {
                    val = $("#DebitCards").find("#" + type+ "_DC")[0].value ;
                }
